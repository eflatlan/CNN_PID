...
vector<string> dig2Clus(const std::string &fileName, vector<Cluster>& clusters, vector<Trigger>& clusterTriggers, vector<Digit>& digits)
{
  ...
  int prev = 0;
  for(const auto& ct : clusterTriggers){
    int si =  ct.getFirstEntry();
    int ei =  ct.getLastEntry();
    if ((ei - si) > prev && (ei-si) < 1000)
    {  
      prev = ei -si;
      for(int i = si; i < ei; i++){
	oneEvtClu.push_back(clusters[i]);
      }
    }
  }
...
cnt++;	   
   ...
   Printf("sze all clus %d, oneevt %d", clusters.size(), oneEvtClu.size());

    Printf("test2");
      for (auto& clus : oneEvtClu) {
      const auto& charge = clus.q();
      const auto& clusSize = clus.size();
      const auto& x = clus.x();
      const auto& y = clus.y();
