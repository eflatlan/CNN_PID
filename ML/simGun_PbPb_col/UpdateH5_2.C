...
masses in std::array<float, 3> masses = {mass_Pion, mass_Kaon, mass_Proton};
// mass_Pion_sq mass_Kaon_sq mass_Proton_sq GeV/c^2
const float mass_Pion_sq = mass_Pion*mass_Pion, mass_Kaon_sq =
mass_Kaon*mass_Kaon,  mass_Proton_sq = mass_Proton*mass_Proton;
...
double calcCkovFromMass(float p, float n, float m)
{
  ...
  const auto cos_ckov = static_cast<float>(TMath::Sqrt(p_sq +
m*m)/(cos_ckov_denom));
...
{
  // Loops on all Ckov candidates and estimates the best Theta Ckov for a ring
formed by those candidates. Also estimates an error for that Theat Ckov
  // collecting errors for all single Ckov candidates thetas. (Assuming they are
independent)
  // Arguments: iNclus- total number of clusters in chamber for background
estimation
  ...
  for (Int_t i = 0; i < fPhotCnt; i++) { // candidates loop
    if (fPhotFlag[i] == 2) {
      if (fPhotCkov[i] < ckovMin) {
        ckovMin = fPhotCkov[i];
      } // find max and min Theta ckov from all candidates within probable
window if (fPhotCkov[i] > ckovMax) { ckovMax = fPhotCkov[i];
      }
      weightThetaCerenkov += fPhotCkov[i] * fPhotWei[i];
      wei += fPhotWei[i]; // collect weight as sum of all candidate weghts

      sigma2 += 1. / fParam->sigma2(fTrkDir.Theta(), fTrkDir.Phi(),
fPhotCkov[i], fPhotPhi[i]);
    }
  } // candidates loop
...
double houghResponse()
{
  ...
  return (Double_t)(locMax * fDTheta + 0.5 * fDTheta); // final most probable
track theta ckov
...
public:
  ...
  double mxRad, myRad, mxMip, myMip, mThetaP, mPhiP, mRefIndex, mMomentum,
      ckovReconstructed, ckovReconstructedMassHyp;
  ...
  ParticleInfo2(float xRad, float yRad, float xMip, float yMip, float th,
      ...
      : mxRad(xRad), myRad(yRad), mxMip(xMip), myMip(yMip), mThetaP(th),
        ...
        ckovReconstructed(ckovRecon), ckovReconstructedMassHyp(0) {}

  void updateValues(double sigmaRing, double thetaCer, double phiCer,
                    double ckovReconMass) {
...
void createOrUpdateDataset(H5::Group &group, const std::string &name,
...
void updateDataset(Group &group, const std::string &name,
                   const std::vector<double> &newValues) {
  DataSet dataset = group.openDataSet(name);
  dataset.write(newValues.data(), PredType::NATIVE_DOUBLE);
}

void updateParams(H5::Group &particleGroup, double momentum, double thetaP,
                  double phiP, double xMip, double yMip, double xRad,
                  double yRad) {
  ...
  */
  ...
  std::vector<double> x_values(num_elements), y_values(num_elements),
      q_values(num_elements), size_values(num_elements),
      sigma_ring_values(num_elements), phi_cer_values(num_elements),
      theta_cer_values(num_elements);
...
datasetPhiCer.read(phi_cer_values.data(), PredType::NATIVE_DOUBLE);*/
  ...
  for (size_t i = 0; i < num_elements; ++i) {
    clusters.emplace_back(x_values[i], y_values[i], q_values[i],
                          size_values[i]);
  }
  ...
  for (size_t i = 0; i < num_elements; ++i) {
    ...
    if (reconObj.findPhotCkov(x_values[i], y_values[i], thetaCer, phiCer)) {
      double sigma2 = alisSigma2.sigma2(
          thetaP, phiP, thetaCer, phiCer); // double trkTheta, double trkPhi,
                                           // double ckovTh, double ckovPh
      ...
      // bool isCand = isPhotonHadronCand(thetaCer, sigma2, momentum,
      // refIndexTmp);
      /*if(isCand) {

fPhotCkov[fPhotCnt] = thetaCer;                               // actual theta
Cerenkov (in TRS) fPhotPhi[fPhotCnt] = phiCer; fPhotClusIndex[fPhotCnt] = iClu;
// actual phi   Cerenkov (in TRS): -pi to come back to "unusual" ref system
(X,Y,-Z) fPhotCnt++;                      // increment counter of photon
candidates

      }*/
      // isPhotonHadronCand(double thetaCer, double sigma, double p, double n)
      /*if (std::abs(thetaCer-pionVal) < 2*sigma2) {
            std::cout << "std::abs(thetaCer-pionVal) < 2*sigma_ring_values[i]"
      << std::endl;
      }*/
...
void writeH5File(const std::string &filename,
                 const std::vector<ParticleInfo2> &particleInfos) {
...
std::vector<ParticleInfo2> readH5File(const std::string &filename) {
  ...
  try {
    ...
    const int num_particles =
        file.getNumObjs(); // Assuming this gets the number of groups
    ...
    for (int i = 0; i < num_particles; ++i) {
      ...
      // ParticleInfo2(float xRad, float yRad, float xMip, float yMip, float th,
      // float ph, float refIndex, int cluCharge, int cluSize, float p, int
      // mcTrackPdg, float ckovRecon)

      double momentum, thetaP, phiP, xMip, yMip, xRad, yRad = 0;

      particleGroup.openAttribute("Momentum")
          .read(PredType::NATIVE_DOUBLE, &momentum);

      particleGroup.openAttribute("ThetaP").read(PredType::NATIVE_DOUBLE,
                                                 &thetaP);
      ...
      updateParams(particleGroup, momentum, thetaP, phiP, xMip, yMip, xRad,
                   yRad);
      // class SimpleCluster
      ...
      // Recon reconObj(double thetaP, double phiP, double xMip, double yMip,
      // double xRad, double yRad, std::vector<SimpleCluster> clustersIn) {
  ...
  // void updateDataset(Group& group, const std::string& name, const
