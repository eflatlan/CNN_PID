...
public:
  void checkCond(const TVector2 &posPhoton, bool above,
    ...
    Printf("phiPhoton %.2f| phiPhoton -  1 %.2f, phiPhoton + 1 %.2f", phiPhoton,
           vec[initValue + iCnt][1], vec[initValue + iCnt + 1][1]);
    ...
    // check if outside of radius
      ...
      while (noDecisionTaken) {
        if ((rPhoton > rMax)) {
                                        ...
                                        // iterate by splitting r1, r2 -> phi1
                                        // phi2

                                        splitPhi(phiMax, phiLmax, phiMin,
                                                 phiLmin, etaCkov,
                                                 phiPhoton); //
                                        rMax = getR(etaCkov, phiLmax, L);
                                        rMin = getR(etaCkov, phiLmin, L);
  ...
  void splitPhi(double &phiMax, double &phiLmax, double &phiMin,
                double &phiLmin, const double &etaCkov, const double &L,
...
if(getPionStatus()) {
  ...
  auto phiL = switchLogic(thetaP, theta, phi, getMaxCkovPion(), phiLvector,
phiVector); TVector2 posEstimatedPhi = tracePhoton(getMaxCkovPion(), phiL,
lMin); //

  auto r = (posEstimatedPhi - trkPC).Mod(); // find R of rMax (dist mip 2 {x,y}
of estimated phi);
...
if(getProtonStatus()) {
  ...
  auto phiL = switchLogic(thetaP, theta, phi, getMinCkovProton(), phiLvector,
phiVector); TVector2 posEstimatedPhi = tracePhoton(getMinCkovProton(), phiL,
lMin); //
