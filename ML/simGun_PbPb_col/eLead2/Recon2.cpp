...
public:
  ...
  Recon2(double thetaP, double phiP, double xMip, double yMip, double xRad,
...
TVector2 fPc;     // track position at PC           // XY*/
...

  {
    ...
    XYZVector rad(fTrkPos.X(), fTrkPos.Y(),
    ...
    while (1) {
      ...
      /*Printf("dirCkov | ckov : %.2f, phi : %.2f  ", dirCkov.Theta(),
      dirCkov.Phi());
      ...
      std::cout << " cluX  " << cluX << " posC.X " << posC.X() << " cluY  "
                << cluY << " posC.Y " << posC.Y() << std::endl;

      std::cout << "ckovTh " << ckov << "  phi  " << phi << "  dist " << dist
                << std::endl;

      std::cout << "thetaCer " << thetaCer << "  phiCer  " << phiCer
                << std::endl;
...

  {
    ...
    if (thetaCer > TMath::ASin(1. / getRefIdx())) {
      Printf("thetaCer %.2f > TMath::ASin(1. / getRefIdx(): %.2f  ", thetaCer,
             TMath::ASin(1. / getRefIdx()));
    ...
    Printf("   propagate2  | dirCkov | ckov : %.2f, phi : %.2f  ",
           dirCkov.Theta(), dirCkov.Phi());

    Printf("   propagate2  | posCkov | x : %.2f, y : %.2f  , z : %.2f ",
           posCkov.X(), posCkov.Y(), posCkov.Z());

    refract(dirCkov, getRefIdx(), winIdx()); // RAD-WIN refraction

    Printf("   refract2  | dirCkov | ckov : %.2f, phi : %.2f  ",
           dirCkov.Theta(), dirCkov.Phi());
...

  {
    ...
    //  dirTrk.SetCoordinates(1,fTrkDir.Theta(),fTrkDir.Phi())
