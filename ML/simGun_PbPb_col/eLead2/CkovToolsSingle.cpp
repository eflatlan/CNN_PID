...
delete 			hSignalAndNoiseMap;
  ...
  void drawTotalMap(std::vector<o2::hmpid::ClusterCandidate> &clusterTrack,
    ...
    // printf("		fillMapFromVec(mArrAndMap->hMaxPion, arrMaxPion);// map,
    // array");
...
private:
  ...
  // std::unique_ptr<ArrAndMap> mArrAndMap;// = nullptr;// new
  // ArrAndMap(eventCnt);
  int trackPdg;
...
public:
  ...
  CkovTools(double radParams[7], double refIndexes[3], double MIP[3],
    ...
    trackPdgString = getPDG(trackPdg);
    // printf(" Track PDG %d %s | CkovTools momentum = %.2f, refFreon = %.2f;
    // ckovHyps : %.2f %.2f %.2f", trackPdg, trackPdgString.c_str(),momentum,
    // nF, ckovHyps[0], ckovHyps[1], ckovHyps[2]);

    // TODO ; later check how to deal w this: set it to highest value in range
    // that makes it valid ?
    ...
    if (getPionStatus()) {
      ...
      // printf("init CkovTools constructor : getPionStatus() true ! minPion
      // %.2f, maxPion %.2f ", ckovPionMin, ckovPionMax);
    } else {
  ...
  segment(std::vector<o2::hmpid::ClusterCandidate> &clusterTrack,
          std::array<int, 4> &arrayInfo, int trackIndex,
          const std::vector<float> &mipCharges, float mipX, float mipY,
    ...
    // L = %.2f \n #Theta_{C} = %.4f maxCkov = %.4f ; x [cm]; y [cm]",
    // thetaP,phiP, L,trackCkov,ckovPionMax);

    // TH2F *localRefMIP = new TH2F("localRefMIP ",
    // infString,800,-40.,-40.,900,-40.,40.);
    ...
    // void setArrayMin(Populate* populate, double etaTRS, vecArray3
    // inPutVector)
    ...
    vecArray4 arrMaxPion, arrMinPion, arrMinProton, arrMaxProton, arrMinKaon,
        arrMaxKaon;

    vecArray2 arrMaxPionPos, arrMinPionPos, arrMinProtonPos, arrMaxProtonPos,
        arrMinKaonPos, arrMaxKaonPos;
    ...
    // check if candidate can be proton (i.e., that it exceeds momentum
    // threshold)
    if (getProtonStatus()) {
      ...
      calculateDifference(arrMaxProtonPos, arrMinProtonPos, getCkovProton(),
                          sigmaProton);
    }
    ...
    if (getKaonStatus()) {
      ...
      Sigma2 sigmaKaon(
          getCkovKaon(), phiP, thetaP,
          nF); //(double thetaC, double phiP, double thetaP, double _nF)
      calculateDifference(arrMaxKaonPos, arrMinKaonPos, getCkovKaon(),
                          sigmaKaon);
    }

    // printf("BF : Length of elem vectors : arrMaxPion %zu",
    // arrMaxPion.size());
    // printf("calling setArrayMax w getMaxCkovPion() = %.2f",
    // getMaxCkovPion());

    Printf("BF : Length of elem vectors : arrMinPionPos %zu",
           arrMinPionPos.size());
    // void calculateDifference(vecArray2& maxVec, vecArray2& minVec, double
    // etaTrsMax, double etaTrsMin) {

    Sigma2 sigmaPion(
        getCkovPion(), phiP, thetaP,
        nF); //(double thetaC, double phiP, double thetaP, double _nF)

    Printf(" getCkovPion() %.4f", getCkovPion());
    calculateDifference(arrMaxPionPos, arrMinPionPos, getCkovPion(), sigmaPion);

    Printf("AFTER : Length of elem vectors : arrMinPionPos %zu",
           arrMinPionPos.size());
    ...
    for (auto &photons : clusterTrack) {
      ...
      const auto &dist = (photons.mX - mipX) * (photons.mX - mipX) +
                         (photons.mY - mipY) * (photons.mY - mipY);
      ...
      if (photons.mX == mipX &&
          photons.mY ==
              mipY /*&& photons.mQ ==  mipCharge*/) { // current cluster is mip
        ...
        if (true) {
          ...
          } // trackIndex, const std::vector<float>& mipCharges, float mipX,
            // float mipY, const int mcmcTrackPdg
        }
      ...
      if (photons.mQ > mipCut && photons.mSize > mipSizeCut &&
          cStatusCkov != 1) {
        ...
        cStatus = 1;
        mArrAndMap.fillmipSizeFilter(x, y);
        // continue; // this photon charge is a MIP--> dont consider as
        // candidate
      }
      ...
      bool isPhotonProtonCand = false, isPhotonKaonCand = false,
           isPhotonPionCand = false;
      bool isPhotonProtonCandCkov = false, isPhotonKaonCandCkov = false,
           isPhotonPionCandCkov = false;
      ...
      if ((rPhoton < rMax && rPhoton > rMin) && cStatusCkov == 0) {
        ...
        if (findPhotCkov(
                ...
                phiCer)) { // find ckov angle for this  photon candidate
          ...
          if (sigmaRing < 0.025) {
            Printf("thetaCer %.4f phiCer %.2f, sigmaRing %.5f", thetaCer,
                   phiCer, sigmaRing);

            // Printf("getCkovPion %.4f getCkovKaon %.4f, getCkovProton %.4f" ,
            // getCkovPion(), getCkovKaon(), getCkovProton());
          }
          ...
          if (thetaCer > TMath::ASin(1. / (nF - 3 * 0.0055))) {
            ...
            if (sigmaRing > 0.2) {
              ...
              if (TMath::Abs(thetaCer - getCkovPion()) <
                  mSigmaSep * sigmaRing) {
                isPhotonPionCandCkov = true; // Printf("ckovPionOk");
                pionCandidates.push_back(std::array<double, 2>{x, y});
              }
              if (TMath::Abs(thetaCer - getCkovKaon()) <
                  mSigmaSep * sigmaRing) {
                isPhotonKaonCandCkov = true; // Printf("ckovKaonOk");
                kaonCandidates.push_back(std::array<double, 2>{x, y});
              }
              if (TMath::Abs(thetaCer - getCkovProton()) <
                  mSigmaSep * sigmaRing) {
                isPhotonProtonCandCkov = true; // Printf("ckovProtonOk");
        ...
        bool isMaxProtonOk = false, isMinProtonOk = false, isMaxKaonOk = false,
             isMinKaonOk = false, isMaxPionOk = false, isMinPionOk = false;
        ...
        if (getProtonStatus() and getKaonStatus() and getPionStatus()) {
          ...
          // printf("\n\npopulate2Ptr->checkUnder(posPhoton, rPhoton, phiPhoton,
          // above, arrMinProton, getMinCkovProton());");

          bool shutDownOnOpen = false;
          // isMinProtonOk = populate2Ptr->checkUnder(posPhoton, rPhoton,
          // phiPhoton, arrMinProton, getMinCkovProton(), "Proton",
          // shutDownOnOpen);

          // printf("isMinProtonOk = evaluatePointContour(posPhoton,
          // arrMinProtonPos, false);");
          // to be true; point needs to be outside of contour of minProton
          isMinProtonOk = evaluatePointContour(
              posPhoton, arrMinProtonPos,
              false); // outside = intersection!
          ...
          /* checkUnder(const TVector2& posPhoton, const double& rPhoton, const
           * double& phiPhoton, vecArray4& vec, const double& etaCkov, const
           * char* hadronType)
           */
          ...
          if (isMinProtonOk) {
            ...
            // printf("\n\npopulate2Ptr->checkOver(posPhoton, rPhoton,
            // phiPhoton, arrMaxPion, getMaxCkovPion());");

            // isMaxPionOk = populate2Ptr->checkOver(posPhoton, rPhoton,
            // phiPhoton, arrMaxPion, getMaxCkovPion(), "Pion");
            // printf("isMaxPionOk = !evaluatePointContour(posPhoton,
            // arrMaxPionPos, true);");

            // to be true; point needs to be inside of contour of isMaxPionOk
            isMaxPionOk = !evaluatePointContour(
                posPhoton, arrMaxPionPos,
                true); // inside = no intersection!
                       // printf("isMaxPionOk %d", isMaxPionOk);
            // this means rProtonMin < rPhoton < rMaxPion
            if (isMaxPionOk) {

              // this means rProtonMax > rPhoton; then also the rPh < rPionMax
              // and rPh < rKaonMax

              // isMaxProtonOk = populate2Ptr->checkOver(posPhoton, rPhoton,
              // phiPhoton, arrMaxProton, getMaxCkovProton(), "Proton");

              // to be true; point needs to be inside of contour of
              // isMaxProtonOk
              // printf("		isMaxProtonOk =
              // !evaluatePointContour(posPhoton,  arrMaxProtonPos, true)");
              isMaxProtonOk = !evaluatePointContour(
                  posPhoton, arrMaxProtonPos,
                  true); // inside = no intersection!
              ...
              // printf("	isMinPionOk = evaluatePointContour(posPhoton,
              // arrMinPionPos, false);");
              // isMinPionOk = populate2Ptr->checkUnder(posPhoton, rPhoton,
              // phiPhoton, arrMinPion, getMinCkovPion(), "Pion",
              // shutDownOnOpen);
              isMinPionOk = evaluatePointContour(
                  posPhoton, arrMinPionPos,
                  false); // printf("isMinPionOk %d", isMinPionOk);
              if (shutDownOnOpen) {
                errorPos.Set(posPhoton);
                // printf("shutDownOnOpen!!");
                drawMap = true;
              ...
              if (!isMinKaonOk) {
                ...
                // isMinKaonOk = populate2Ptr->checkUnder(posPhoton, rPhoton,
                // phiPhoton, arrMinKaon, getMinCkovKaon(), "Kaon",
                // shutDownOnOpen);
                // printf("isMinKaonOk = evaluatePointContour(posPhoton,
                // arrMinKaonPos, false);");
                isMinKaonOk = evaluatePointContour(
                    posPhoton, arrMinKaonPos,
                    false); // printf("isMinKaonOk %d", isMinKaonOk);
              ...
              if (isMinKaonOk) {
                if (!isMaxKaonOk) {
                  // printf("isMaxKaonOk = !evaluatePointContour(posPhoton,
                  // arrMaxKaonPos, true)");
                  // isMaxKaonOk = populate2Ptr->checkOver(posPhoton, rPhoton,
                  // phiPhoton, arrMaxKaon, getMaxCkovKaon(), "Kaon");
                  isMaxKaonOk =
                      !evaluatePointContour(posPhoton, arrMaxKaonPos,
                                            true); // inside = no intersection!
        ...
        // if Proton not is possible because momentum-threshold is not exceeded:
          ...
          bool shutDownOnOpen = false;
          // printf("isMinKaonOk = evaluatePointContour(posPhoton,
          // arrMinKaonPos, false)"); isMinKaonOk =
          // populate2Ptr->checkUnder(posPhoton, rPhoton, phiPhoton, arrMinKaon,
          // getMinCkovKaon(), "Kaon", shutDownOnOpen);
          isMinKaonOk = evaluatePointContour(posPhoton, arrMinKaonPos,
                                             false); // outside =  intersection!
          if (shutDownOnOpen) {
            errorPos.Set(posPhoton);
            // printf("shutDownOnOpen!!");
          ...
          if (isMinKaonOk) {
            ...
            // arrMaxPionPos, true)");
            isMaxPionOk = !evaluatePointContour(
                posPhoton, arrMaxPionPos, true); // inside =  no intersection!
            if (isMaxPionOk) {
              bool shutDownOnOpen = false;
              // isMinPionOk = populate2Ptr->checkUnder(posPhoton, rPhoton,
              // phiPhoton, arrMinPion, getMinCkovPion(), "Pion",
              // shutDownOnOpen); printf("isMinPionOk =
              // evaluatePointContour(posPhoton,  arrMinPionPos, false);");
              isMinPionOk = evaluatePointContour(
                  posPhoton, arrMinPionPos, false); // outside =  intersection!
              ...
              // printf("isMaxKaonOk = !evaluatePointContour(posPhoton,
              // arrMaxKaonPos, true)");
              isMaxKaonOk = !evaluatePointContour(
                  posPhoton, arrMaxKaonPos, true); // inside =  no intersection!
              ...
              // printf("Photon%d could be Pion/Kaon (from p-hyp) but was out of
              // radius-range",iPhotCount);
            }
          } // end if isMinKaonOk
          else {
            // printf("Photon%d could be Pion/Kaon (from p-hyp) but was out of
        ...
        } // end else if (!getProtonStatus() and getKaonStatus() and
          // getPionStatus())

        // denne ser litt rar ut?

        // if neither Proton or Kaon is possible because momentum-threshold is
        // not exceeded:
        else if (!getProtonStatus() and !getKaonStatus() and getPionStatus()) {
          // printf("Photon%d can be Pion from p-hyp", iPhotCount);

          // printf("populate2Ptr->checkUnder(posPhoton, rPhoton, phiPhotob,
          // arrMaxPion, getMaxCkovPion());");
          ...
          if (isMaxPionOk) {
            ...
            // printf("populate2Ptr->checkOver(posPhoton, rPhoton, phiPhoton,
            // arrMinPion, getMaxCkovPion());",iPhotCount);

            bool shutDownOnOpen = false;
            // bool isMinPionOk = populate2Ptr->checkUnder(posPhoton, rPhoton,
            // phiPhoton, arrMinPion, getMinCkovPion(), "Pion", shutDownOnOpen);
            bool isMinPionOk = evaluatePointContour(
                posPhoton, arrMinPionPos, false); // outisde = intersection!
            ...
            if (isMinPionOk) {
              ...
              // printf("Photon%d could only have been Pion, but didnt fall
              // within radius-range",iPhotCount);
            }
          } else {
            // printf("Photon%d could only have been Pion, but didnt fall within
            // radius-range",iPhotCount);
          }
        ...
        // this should not really be "possible", but can be if we have another
        // candidate that is not pion/kaon/proton
        else {
          // printf("Photon%d was not Pion Kaon or Proton from
          // p-hyp",iPhotCount);
          //  we got particle that should not be able to be pion, kaon or proton
        }

        /*if (isPhotonPionCandCkov || isPhotonKaonCandCkov ||
 isPhotonProtonCandCkov) { printf("\n======================"); printf("	Photon%d
 Candidate Ckov: Pion = %d Kaon = %d Proton %d", iPhotCount,
 isPhotonPionCandCkov, isPhotonKaonCandCkov, isPhotonProtonCandCkov); printf("
 Photon%d  Candidate segm: Pion = %d Kaon = %d Proton %d", iPhotCount,
 isPhotonPionCand, isPhotonKaonCand, isPhotonProtonCand);
        ...
        if (cStatusCkov == 0) {
          ...
          // printf("	Photon%d  Candidate Ckov: pionBit = %d kaonBit = %d
          // protonBit %d", iPhotCount, pionBit, kaonBit, protonBit);
        }

        if (cStatus == 0) {
          cStatus = 1 + 4 * static_cast<int>(isPhotonPionCand) +
                    2 * static_cast<int>(isPhotonKaonCand) +
        ...
        // o2::hmpid::ClusterCandidate :setCandidateStatus(int iTrack, int
        // hadronCandidateBit)
        photons.setCandidateStatus(cStatus);

        photons.setCandidateStatusCkov(cStatusCkov);

        // LOGP(info, "Set Track {} : cStatusCkov {} | cStatus {} ", trackIndex,
        // cStatusCkov, cStatus);
      ...
      // radius was to big to consider :
        ...
        photons.setCandidateStatusCkov(cStatusCkov);
        // LOGP(info, "Radius {} too high ! cStatusCkov {}", rPhoton,
        // cStatusCkov);
...
if(numFoundActualCkov != numActualCkov) {
    ...
    // printf("=========================================================================");
    // printf("CkovHyps, possible candidates from Momentum | Pion = %d, Kaon =
    // %d, Proton = %d", getPionStatus(), getKaonStatus(), getProtonStatus());

    /*int cntTemp = 0;
  for(const auto& c : candidatesCombined){
                ////printf("Phot%d : x = %.2f, y = %.2f || statusCand = %d",
  cntTemp++, c.x, c.y, c.candStatus);
        }*/
    // printf("=========================================================================");

    // printf("number of candidates : proton %zu, kaon %zu, pion %zu | total
    // %zu, radius over Limit %d ", protonCandidates.size(),
    // kaonCandidates.size(), pionCandidates.size(), clusterTrack.size(),
    // rOverMax);

    // printf("MIP(%.1f %.1f) RAD (%.1f %.1f) Track (%.1f %.1f)", mipX, mipY,
    // xRad, yRad, thetaP, phiP);

    // for(const auto& pair: filledBins)
    //	//printf("CkovTools segment candidates: x%f y%f", pair.first,
    //pair.second);
    ...
    /*
     ...
     segm->cd(2);
     mapPion->SetTitle(Form("mapPion : min%d max%d photons%d",
     mapPionMin->GetEntries(), mapPionMax->GetEntries(),
     mapPhotons->GetEntries())); mapPion->Draw(); mapPhotons->Draw("same");
     localRefMIP2->Draw("same");
     ...
     mapKaon->SetTitle(Form("mapKaon: min%d max%d photons%d",
     mapKaonMin->GetEntries(), mapKaonMax->GetEntries(),
     mapPhotons->GetEntries())); mapKaon->Draw(); mapPhotons->Draw("same");
     mapKaonMin->Draw("same");
     mapKaonMax->Draw("same");
     trkPCMap->Draw("same"); trkRadMap->Draw("same");

     segm->cd(4);
     mapProton->SetTitle(Form("mapProton : min%d max%d photons%d",
     mapProtonMin->GetEntries(), mapProtonMax->GetEntries(),
     mapPhotons->GetEntries())); mapProton->Draw();
    ...
    if (/*drawMap*/ false) {
      ...
      if (false) { // printf("populate2Ptr was nullptr!");
        ...
        Printf("SigmaSep %.2f  | nPi %d nK %d nPr %d == nT %d | PDG %d | "
               "thetaP %.2f phiP %.2f",
               mSigmaSep, numPion, numKaon, numProton, numTotal, trackPdg,
               thetaP, phiP);
        ...
        // printf("populatePtr->GetHistogram()->GetEntries() num BinEntries = %f
        // ", populate2Ptr->getLimMin()->GetEntries()); printf("mArrAndMap->//
        // limMin->GetEntries() num BinEntries = %f ", populate2Ptr->//
        // limMin->GetEntries());
        // to draw the "split-phi"...
        // mArrAndMap.setDrawLimits(populate2Ptr->// limMin, populate2Ptr->//
        // limMax);

        // printf("mArrAndMap->// limMin->GetEntries() num BinEntries = %f ",
        // populate2Ptr->// limMin->GetEntries());
        ...
        // printf("delta %.2f | x %.2f y %.2f ", d*tanP, d*tanP*cosP,
        // d*tanP*sinP); mArrAndMap->drawTotalMapAndMaxRegions();

        mArrAndMap.drawTotalMap(clusterTrack, plotNumber, xMip, yMip,
                                pionCandidates, kaonCandidates,
                                protonCandidates, canCombined, trackPdg,
                                allCand, mSigmaSep);
        // to drqw the maps ::
        ...
        // printf("\n\n\nEvent Number%d, momentum %.2f | mass %.2f | thetaP %.2f
        // | phiP %.2f | L %.2f", eventCnt,momentum, mass, thetaP, phiP, L);
        ...
        // printf("phiP %.5f: angle rad --> pc = %.5f | Acos %.5f Asin %.5f"  ,
        // phiP, (trkPos-trkPC).Phi(),
        // TMath::ACos(d*tanP*cosP/(trkPos-trkPC).Mod()),
        // TMath::ASin(d*tanP*sinP/(trkPos-trkPC).Mod()));

        // printf("PHI : l1 %.5f | l2 %.5f ",
        // (l1-trkPos).Phi(),(l2-trkPos).Phi()); printf("RADIUS : l1 %.5f | l2
        // %.5f ", (l1-trkPC).Mod(),(l2-trkPC).Mod());

        // printf("CkovHyps %.2f %.2f %.2f", ckovHyps[0], ckovHyps[1],
        // ckovHyps[2]); Printf("CkovHyps %.2f %.2f %.2f", ckovHyps[0],
        // ckovHyps[1], ckovHyps[2]);
     ...
     */
      ...
      // arrayInfo = {numBackgroundPhotons, numFoundActualCkov, numActualCkov,
  ...
  bool lineIntersects(const TVector2 &B, const std::array<double, 2> &C,
                      const std::array<double, 2> &D, double rUncertainty,
                      TVector2 &intersection, bool &pointIsInside) {
    ...
    bool isOutOfBounds =
        (x < std::min(mipPos.X(), B.X()) || x > std::max(mipPos.X(), B.X()) ||
    ...
    if (isOutOfBounds) {
      ...
      TVector2 adjustedIntersection =
          intersection + unitDirMIPtoIntersection * rUncertainty;
      ...
      if (isOutOfBounds) {
        // Printf("dlineIntersects :: adjusted point is still out of bounds,
        // returning false");
        return false; // Adjusted intersection is still out of bounds
      } else {
        intersection = adjustedIntersection;
        pointIsInside =
            (adjustedIntersection - mipPos).Mod() < (B - mipPos).Mod();
        // printf("OK :: dist2 Point %.2f | dist2 adj %.2f |  adj %.2f", dist1,
        // dist2, rUncertainty);
      }
    } else {
      ...
      // printf("dist2 Point %.2f | dist2 adj %.2f", dist1, dist2,
      // rUncertainty);
    }
  ...
  bool evaluatePointContour(const TVector2 &posPhoton, const vecArray2 &contour,
                            bool checkInside) {
    ...
    double d =
        (posPhoton.X() - shortSidePoint[0]) * (mipPos.Y() - shortSidePoint[1]) -
        (posPhoton.Y() - shortSidePoint[1]) * (mipPos.X() - shortSidePoint[0]);

    if (d > 0) {
      for (size_t i = 0; i < contour.size() - 1; ++i) {
        size_t next = i + 1;
        if (lineIntersects(posPhoton, contour[i], contour[next], rUncertainty,
                           intersection, pointIsInside)) {
          intersects = true;
          if (pointIsInside)
            return true; // Point is inside the contour
          else
            return false; // Point is outside the contour
        }
      }
    } else {
      for (size_t i = contour.size() - 1; i > 0; --i) {
        size_t prev = i - 1;
        if (lineIntersects(posPhoton, contour[i], contour[prev], rUncertainty,
                           intersection, pointIsInside)) {
          intersects = true;
          if (pointIsInside)
            return true; // Point is inside the contour
    ...
    // printf("evaluatePointContour checkInside %d  : pointIsInside == %d",
    // checkInside, pointIsInside);
    return intersects;
  }

  // Trace a single Ckov photon from emission point somewhere in radiator up to
  // photocathode taking into account ref indexes of materials it travereses
  // Arguments: ckovThe,ckovPhi- photon ckov angles in TRS, [rad]
  //   Returns: distance between photon point on PC and track projection
  // placeholder...
  void setArrayMax(double etaTRS, vecArray4 &inPutVectorAngle,
                   vecArray2 &inPutVectorPos, const size_t kN) {
    ...
    for (int i = 0; i < kN; i++) {
      ...
      // Printf("\n setArrayMax() dirTrs.SetMagThetaPhi(1, etaTRS = x %.2f, phiL
      // = x %.2f); ", etaTRS, phiL);
      ...
      // Printf("setArrayMax() called trs2Lors on dirTRS : return { thetaR =
      // %.2f, phiR = %.2f}", thetaR, phiR);

      dirLORS.SetMagThetaPhi(1, thetaR, phiR);

      // Printf("setArrayMax() dirLORS {x %.2f y %.2f z %.2f}", dirLORS.X(),
      // dirLORS.Y(), dirLORS.Z());
      ...
      // Printf("setArrayMax() called  populatePtrOuter->traceForward(dirLORS
      // (thetaR %.2f, phiR %.2f) lMin =  %.2f", thetaR, phiR, lMin);
      const auto &max = populatePtrOuter->traceForward(dirLORS, lMin);
      // max = pos of tracked photon at PC

      const auto r =
          (max - trkPC2).Mod(); // trkPC2 : track impact @ PC
                                // from alinot_paattrec : this is MIP2photon
                                // distance? const auto r = (max -
                                // trkPC2).Mod(); // trkPC2 : track impact @ PC

      auto phiPC = (max - trkPC2).Phi(); // MIP phi value/ PC track value?
      // vector fra photon til MIP/PC (--> verdi som kommer fra phiL)

      // Printf("setArrayMax() traceForward returned TVector2 {x %.2f y %.2f} ==
      // > R  = %.2f", max.X(), max.Y(), r); Printf("setArrayMax() tracePhot
      // returned TVector2 {x %.2f y %.2f} == > R  = %.2f", t.X(), t.Y(),
      // (t-trkPC2).Mod());

      // add protection if traceForward returns 0 or -999?

      // Printf("setArrayMax() --> i %d, {x %.2f y %.2f} - MIP {x %.2f y %.2f}",
      // i, max.X(), max.Y(), trkPC.X(), trkPC.Y());

      // check at phiR  er det samme som (t-rad).phi og (max-rad).phi?

      // Printf("setArrayMax2() --> i %d, {x %.2f y %.2f} - MIP {x %.2f y
      // %.2f}", i, max.X(), max.Y(), trkPC2.X(), trkPC2.Y());
      ...
      /*if( (max-trkRad2).Phi() != (t-trkRad2).Phi()) {
              throw std::invalid_argument("(max-trkRad2).Phi() !=
      (t-trkRad2).Phi())");
      }

      if(TMath::Abs(phiR - (t-trkRad2).Phi()) > 0.0001 && t.X() != -999 && t.Y()
      != -999)
      {

        Printf("phiR at %.6f | (t-rad) %.6f | (max-rad) %.6f ", phiR,
      (t-trkRad2).Phi(), (max-trkRad2).Phi()); throw std::invalid_argument("phiR
      != (t-trkRad2).Phi())");
      }
      ...
      if ((max.Y() == -999) or (max.X() == -999)) {
        ...
        inPutVectorAngle.emplace_back(
            std::array<double, 4>{phiL, phiR, phiPC, r});
        ...
        // Printf("setArrayMax() emplacing element %d : phiL %.2f, phiR %.2f, r
        // %.2f", i, phiL, phiR, r);
      }
     ...
     for(const auto& ip : inPutVectorAngle) {
             ...
             const auto& r_ = ip[2];
             //Printf("setArrayMax() --> checking inputVector | : phiL %.2f,
phiR %.2f, r %.2f", phiL_, phiR_, r_); if(r_ == 0 ) {throw
std::invalid_argument("r====??????;");}
} */
  }

  double calculatePhi(const std::array<double, 2> &a,
  ...
  // chane function name
  void calculateDifference(vecArray2 &maxVec, vecArray2 &minVec,
                           double ckovHypVal, const Sigma2 &sigma) {
    // vecArray2 calculateDifference(const vecArray2& maxVec, const vecArray2&
    // minVec) {
    ...
    for (size_t i = 0; i < kN; ++i) {
      ...
      // Printf(" phiP TRS_deg %.2f | thetaP_deg %.2f ckovHypVal %.2f | sigma
      // %.5f sigma2Ali %.5f ", phiL*180/3.14, thetaP*180/3.14,  ckovHypVal,
      // sigma2, sigma2Ali);
  ...
  double getTotalSigmaPos(double sigmaPositions) {
    ...
    Printf(" getTotalSigmaPos : sigmaPositionssq %.3f  sigmaLsq %.23f sigmaRsq "
           "%.3f",
           sigmaPositions * sigmaPositions, sigmaLsq, sigmaRsq);
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  Double_t aliSigma2(Double_t trkTheta, Double_t trkPhi, Double_t ckovTh,
                     Double_t ckovPh) {
    // Analithical calculation of total error (as a sum of localization,
    // geometrical and chromatic errors) on Cerenkov angle for a given Cerenkov
    // photon created by a given MIP. Fromulae according to CERN-EP-2000-058
    ...
    if (sigRMS < 0.00000002)
      Printf("SigLoc %.5f, SigGeom %.5f, z (SigCrom): %.5f  == %.5f (sigRMS): "
             "| Inputs: trkTheta: %.5f, trkPhi: %.5f, ckovTh: %.5f, ckovPh: "
             "%.5f, trkBeta: %.5f",
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  Double_t SigLoc(Double_t trkTheta, Double_t trkPhi, Double_t thetaC,
                  Double_t phiC, Double_t betaM) {
    // Analitical calculation of localization error (due to finite segmentation
    // of PC) on Cerenkov angle for a given Cerenkov photon created by a given
    // MIP. Fromulae according to CERN-EP-2000-058 Arguments: Cerenkov and
    // azimuthal angles for Cerenkov photon, [radians]
    //            dip and azimuthal angles for MIP taken at the entrance to
    //            radiator, [radians] MIP beta
    //   Returns: absolute error on Cerenkov angle, [radians]
    ...
    Double_t k =
        1. - GetRefIdx() * GetRefIdx() +
        alpha * alpha / (betaM * betaM); // formula (after 8 in the text)
    if (k < 0)
      return 1e10;
    Double_t mu = sint * sinf + tantheta * (cost * cosfd * sinf +
                                            sinfd * cosf); // formula (10)
    Double_t e = sint * cosf +
                 tantheta * (cost * cosfd * cosf - sinfd * sinf); // formula (9)

    Double_t kk =
        betaM * TMath::Sqrt(k) / (GapThick() * alpha); // formula (6) and (7)
    Double_t dtdxc =
        kk * (k * (cosfd * cosf - cost * sinfd * sinf) -
              (alpha * mu / (betaM * betaM)) * sint * sinfd); // formula (6)
    Double_t dtdyc = kk * (k * (cosfd * sinf + cost * sinfd * cosf) +
                           (alpha * e / (betaM * betaM)) * sint *
                               sinfd); // formula (7)            pag.4

    Double_t errX = 0.2, errY = 0.25; // end of page 7
    return TMath::Sqrt(errX * errX * dtdxc * dtdxc +
                       errY * errY * dtdyc * dtdyc);
  }
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  Double_t SigCrom(Double_t trkTheta, Double_t trkPhi, Double_t thetaC,
                   Double_t phiC, Double_t betaM) {
    // Analitical calculation of chromatic error (due to lack of knowledge of
    // Cerenkov photon energy) on Cerenkov angle for a given Cerenkov photon
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  Double_t SigGeom(Double_t trkTheta, Double_t trkPhi, Double_t thetaC,
                   Double_t phiC, Double_t betaM) {
    // Analitical calculation of geometric error (due to lack of knowledge of
    // creation point in radiator) on Cerenkov angle for a given Cerenkov photon
    ...
    Double_t eTr = 0.5 * RadThick() * betaM * TMath::Sqrt(k) /
    ...
    Double_t c2 = betaM * TMath::Power(k, 1.5) * tantheta * lambda /
                  (GapThick() * alpha * alpha); // formula (13.b)
    Double_t c3 = (1. + eTr * k * betaM * betaM) /
                  ((1 + eTr) * alpha * alpha); // formula (13.c)
    Double_t c4 = TMath::Sqrt(k) * tantheta * (1 - lambda) /
  ...
  //   Returns: distance between photon point on PC and track projection
  // placeholder...
  void setArrayMin(double etaTRS, vecArray4 &inPutVectorAngle,
                   vecArray2 &inPutVectorPos, const size_t kN) {
    // const size_t kN = inPutVector.size();
    const float lMax = 1.5;
    const auto trkPC2 = populatePtrInner->getPcImp();     // track at PC
    ...
    for (int i = 0; i < kN; i++) {
      ...
      // Printf("setArrayMax() called  populatePtrInner->traceForward(dirLORS
      // (thetaR %.2f, phiR %.2f) lMin =  %.2f", thetaR, phiR, lMin);
      const auto &max = populatePtrInner->traceForward(dirLORS, lMax);
      // max = pos of tracked photon at PC
...
std::invalid_argument("r====??????;");}
  ...
  void setArrayMin2(Populate* populate, double etaTRS, vecArray3& inPutVector)
  {
    ...
    const auto lMax = 1.5;
          ...
          for(int i = 0; i < kN; i++){
                  ...
                  TVector3 dirTrs, dirLORS;
                  dirTrs.SetMagThetaPhi(1, etaTRS, phiL);
                  double thetaR, phiR; // phiR is value of phi @ estimated R in
  LORS populatePtr->trs2Lors(dirTrs, thetaR, phiR); dirLORS.SetMagThetaPhi(1,
  thetaR, phiR);
  ...
  void populateRegions(std::vector<std::pair<double, double>> &vecArr,
                       TH2F *map, const double &eta, const double &l) {
    const int kN = vecArr.size();
    for (int i = 0; i < vecArr.size(); i++) {
      const auto &value =
          populatePtr->tracePhot(eta, double(TMath::TwoPi() * (i + 1) / kN), l);
      if (/*value.X() > 0 && value.X() < 156.0 && value.Y() > 0 && value.Y() <
             144*/
          true) {
  ...
  Double_t WinIdx() const {
    return 1.583;
  } // Mean refractive index of WIN material (SiO2)
  ...
  bool findPhotCkov(double cluX, double cluY, double &thetaCer,
    ...
    */
    ...
    double cluR = TMath::Sqrt(
        (cluX - mipPos.X()) * (cluX - mipPos.X()) +
        (cluY - mipPos.Y()) *
            (cluY - mipPos.Y()));  // ref. distance impact RAD-CLUSTER
    ...
    while (1) {
      ...
      TVector2 posC = populatePtrInner->traceForward(
          dirCkov, 0.75); // trace photon with actual angles
      double dist =
          cluR -
          (posC - mipPos)
