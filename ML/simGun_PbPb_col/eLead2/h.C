...
// /root/alice/sw/slc8_x86-64/ROOT/v6-28-02-7/bin/root.exe By ROOT
// version 6.28/02 on 2023-07-06 11:23:23
...
void h() {
  ...
  // main frame
  TGMainFrame *fRootCanvas284 =
      new TGMainFrame(gClient->GetRoot(), 10, 10, kMainFrame | kVerticalFrame);

  // menu bar
  TGMenuBar *fMenuBar294 =
      new TGMenuBar(fRootCanvas284, 800, 20, kHorizontalFrame);

  // "File" menu
  TGPopupMenu *fPopupMenu286 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 110, 130, kOwnBackground);
  ...
  // cascaded menu "Save"
  TGPopupMenu *fPopupMenu285 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 118, 150, kOwnBackground);
  ...
  fMenuBar294->AddPopup(
      "&File", fPopupMenu286,
      new TGLayoutHints(kLHintsLeft | kLHintsTop, 0, 4, 0, 0));

  // "Edit" menu
  TGPopupMenu *fPopupMenu288 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 69, 130, kOwnBackground);
  ...
  // cascaded menu "Clear"
  TGPopupMenu *fPopupMenu287 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 74, 38, kOwnBackground);
  ...
  // "View" menu
  TGPopupMenu *fPopupMenu290 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 122, 162, kOwnBackground);
  ...
  TGPopupMenu *fPopupMenu289 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 76, 38, kOwnBackground);
  ...
  // "Options" menu
  TGPopupMenu *fPopupMenu291 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 148, 194, kOwnBackground);
  ...
  fMenuBar294->AddPopup(
      "&Options", fPopupMenu291,
      new TGLayoutHints(kLHintsLeft | kLHintsTop, 0, 4, 0, 0));

  // "Tools" menu
  TGPopupMenu *fPopupMenu292 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 120, 102, kOwnBackground);
  ...
  // "Help" menu
  TGPopupMenu *fPopupMenu293 =
      new TGPopupMenu(gClient->GetDefaultRoot(), 120, 142, kOwnBackground);
  ...
  fMenuBar294->AddPopup("&Help", fPopupMenu293,
                        new TGLayoutHints(kLHintsRight | kLHintsTop));
  fRootCanvas284->AddFrame(
      fMenuBar294,
      new TGLayoutHints(kLHintsLeft | kLHintsTop | kLHintsExpandX, 0, 0, 1, 1));
  TGHorizontal3DLine *fHorizontal3DLine302 =
      new TGHorizontal3DLine(fRootCanvas284, 1600, 2);
  fRootCanvas284->AddFrame(fHorizontal3DLine302,
  ...
  fRootCanvas284->AddFrame(fDockableFrame303,
                           new TGLayoutHints(kLHintsExpandX));
  TGHorizontal3DLine *fHorizontal3DLine308 =
      new TGHorizontal3DLine(fRootCanvas284, 1600, 2);
  fRootCanvas284->AddFrame(fHorizontal3DLine308,
                           new TGLayoutHints(kLHintsTop | kLHintsExpandX));

  // composite frame
  TGCompositeFrame *fCompositeFrame309 =
      new TGCompositeFrame(fRootCanvas284, 800, 514, kHorizontalFrame);

  // composite frame
  TGCompositeFrame *fCompositeFrame310 =
      new TGCompositeFrame(fCompositeFrame309, 175, 778, kFixedWidth);
  fCompositeFrame310->SetLayoutManager(
      new TGVerticalLayout(fCompositeFrame310));

  fCompositeFrame309->AddFrame(fCompositeFrame310,
                               new TGLayoutHints(kLHintsLeft | kLHintsExpandY));

  // canvas widget
  TGCanvas *fCanvas311 =
      new TGCanvas(fCompositeFrame309, 800, 514, kSunkenFrame);
  ...
  TGCompositeFrame *fCompositeFrame321 =
      new TGCompositeFrame(gClient, winC, fViewPort312);
  fViewPort312->AddFrame(fCompositeFrame321);
  fCompositeFrame321->SetLayoutManager(
      new TGVerticalLayout(fCompositeFrame321));
  fCompositeFrame321->MapSubwindows();
  fCanvas311->SetContainer(fCompositeFrame321);
  fCanvas311->MapSubwindows();
  fCompositeFrame309->AddFrame(
      fCanvas311,
      new TGLayoutHints(kLHintsRight | kLHintsExpandX | kLHintsExpandY));

  fRootCanvas284->AddFrame(fCompositeFrame309,
  ...
  fRootCanvas284->AddFrame(
      fStatusBar324,
      new TGLayoutHints(kLHintsLeft | kLHintsBottom | kLHintsExpandX, 2, 2, 1,
                        1));
  ...
  fRootCanvas284->SetMWMHints(kMWMDecorAll, kMWMFuncAll, kMWMInputModeless);
