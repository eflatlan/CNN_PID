...
template <typename T>
bool Recon::findPhotCkov(double cluX, double cluY, double &thetaCer,
                         double &phiCer) {
  ...
  double cluR =
      TMath::Sqrt((cluX - fTrkPos.X()) * (cluX - fTrkPos.X()) +
                  (cluY - fTrkPos.Y()) *
...
o2::math_utils::Vector2D<double>
Recon::traceForward(Polar3DVector dirCkov) const {
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void Recon::lors2Trs(Polar3DVector dirCkov, double &thetaCer,
                     double &phiCer) const {
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void Recon::trs2Lors(math_utils::Vector3D<double> dirCkov, double &thetaCer,
                     double &phiCer) const {
  // Theta Cerenkov reconstruction
...
void Recon::findRingGeom(double ckovAng, int level) {
  ...
  for (int i = 0; i < kN; i++) {
    if (!first) {
      pos1 = tracePhot(ckovAng,
                       double(TMath::TwoPi() * (i + 1) /
                              kN)); // find a good trace for the first photon
      if (pos1.X() == -999) {
        continue; // no area: open ring
      }
      if (!fParam->isInside(pos1.X(), pos1.Y(), 0)) {
        pos1 =
            intWithEdge(fMipPos, pos1); // find the very first intersection...
      } else {
        if (!Param::isInDead(1.0f,
                             1.0f)) { // ef: moved method from Param.cxx to h
    ...
    math_utils::Vector2D<double> pos2 =
        tracePhot(ckovAng, double(TMath::TwoPi() * (i + 1) /
                                  kN)); // trace the next photon
    if (pos2.X() == -999) {
      continue; // no area: open ring
    }
    ...
    area += TMath::Abs((pos1 - fMipPos).X() * (pos2 - fMipPos).Y() -
                       (pos1 - fMipPos).Y() *
                           (pos2 - fMipPos).X()); // add area of the triangle...
    pos1 = pos2;
  }
...
// template <typename T>
void Recon::propagate(const Polar3DVector &dir,
                      math_utils::Vector3D<double> &pos, double z) const {
