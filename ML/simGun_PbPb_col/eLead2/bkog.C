...
void backgroundStudy(Int_t NumberOfEvents, Int_t NumberOfClusters,
                     Double_t Hwidth, double ThetaP = .15) {
  Double_t PhiP = 0, PhiF = 0, DegThetaP = 0, DegPhiP = 0;
  ...
  Float_t Xpi = 0, Ypi = 0, Xf = 0, Yf = 0, Xf1 = 0, Yf1 = 0, Xf2 = 0, Yf2 = 0,
          Xp = 0, Yp = 0;
  ...
  TH1F *hTheta = new TH1F(
      "Background", "Background; angle [rad]; counts/1 mrad", 750, 0., 0.75);

  TH1F *hTheta2 =
      new TH1F("Photon Cherenkov Angle Distribution",
               "Photon Cherenkov Angle Distribution;angle [rad]; counts/1 mrad",
               750, 0., 0.75);
  TH1F *hThetawg =
      new TH1F("Track Cherenkov Angle Distribution",
               "Track Cherenkov Angle Distribution;angle [rad]; counts/1 mrad",
               750, 0., 0.75);
  TH1F *hThetaRing = new TH1F(
      "hThetaRing", "Ring Cherenkov; Cherenkov Angle [rad];", 1000, 0., 1.);

  TH2F *hClusterMap = new TH2F("Cluster Map", "Cluster Map; x [Pads]; y [Pads]",
                               1000, -10., 10., 1000, -10., 10.);
  TH2F *hClusterMap2 =
      new TH2F("Cluster Map2", "Cluster Map2; x [Pads]; y [Pads]", 1000, -25.,
               25., 1000, -25., 25.);

  TH2F *hphotonMap = new TH2F("Photon Map", "Photon Map; x [Pads]; y [Pads]",
                              1000, -10., 10., 1000, -10., 10.);

  TH2F *hphotonMap2 = new TH2F("Photon Map2", "Photon Map2; x [Pads]; y [Pads]",
                               1000, -25., 25., 1000, -25., 25.);

  TH2F *signalMap =
      new TH2F("signalHitMap", "signalHitMap", 1000, 0., 1., 1000, 0., 1.);
  TH2F *noiseMap =
      new TH2F("noiseHitMap", "noiseHitMap", 1000, 0., 1., 1000, 0., 1.);
  ...
  Float_t ThetaCherenkov[100000] = {0x0}, PhiCherenkov[100000] = {0x0},
  ...
  for (Int_t iEvt = 0; iEvt < NumberOfEvents; iEvt++) {
    ...
    for (Int_t n1 = 0; n1 < NumberOfClusters; n1++) { // clusters loop
      ...
      //      Xcen[n1] = 130*gRandom->Rndm(n1); Ycen[n1] =
      //      130*gRandom->Rndm(n1);

      TVector3 v2(Xcen[n1] - Xpi -
                      EmissionLenght * TMath::Tan(ThetaP) * TMath::Cos(PhiP),
                  Ycen[n1] - Ypi -
                      EmissionLenght * TMath::Tan(ThetaP) * TMath::Sin(PhiP),
                  RadiatorWidth + QuartzWindowWidth + CH4GapWidth -
                      EmissionLenght);
      ...
      Double_t ThetaF0 =
          TMath::ASin(QuartzIndexOfRefraction / FreonIndexOfRefraction *
                      TMath::Sin(ThetaLimite)) -
          0.00001;

      //  Printf("ThetaF0 = %f",ThetaF0*TMath::RadToDeg());

      Double_t ThetaF01 =
          TMath::ASin((FreonIndexOfRefraction / QuartzIndexOfRefraction) *
                      (TMath::Sin(ThetaF0)));

      Double_t ThetaF02 =
          TMath::ASin((QuartzIndexOfRefraction / CH4IndexOfRefraction) *
                      (TMath::Sin(ThetaF01)));
      ...
      Float_t X02 =
          (RadiatorWidth - EmissionLenght) * TMath::Tan(ThetaF0) *
              TMath::Cos(PhiF) +
          QuartzWindowWidth * TMath::Tan(ThetaF01) * TMath::Cos(PhiF) +
          CH4GapWidth * TMath::Tan(ThetaF02) * TMath::Cos(PhiF);

      Float_t Y02 =
          (RadiatorWidth - EmissionLenght) * TMath::Tan(ThetaF0) *
              TMath::Sin(PhiF) +
          QuartzWindowWidth * TMath::Tan(ThetaF01) * TMath::Sin(PhiF) +
          CH4GapWidth * TMath::Tan(ThetaF02) * TMath::Sin(PhiF);
      ...
      while (TMath::Sqrt((Xf - Xcen[n1] + Xpi) * (Xf - Xcen[n1] + Xpi) +
                         (Yf - Ycen[n1] + Ypi) * (Yf - Ycen[n1] + Ypi)) >
             0.0001)

      {
        ...
        ThetaF2 = TMath::ASin((QuartzIndexOfRefraction / CH4IndexOfRefraction) *
                              (TMath::Sin(ThetaF1)));
        ...
        Xf2 = (RadiatorWidth - EmissionLenght) * TMath::Tan(ThetaF) *
                  TMath::Cos(PhiF) +
              QuartzWindowWidth * TMath::Tan(ThetaF1) * TMath::Cos(PhiF) +
              CH4GapWidth * TMath::Tan(ThetaF2) * TMath::Cos(PhiF);

        Yf2 = (RadiatorWidth - EmissionLenght) * TMath::Tan(ThetaF) *
                  TMath::Sin(PhiF) +
              QuartzWindowWidth * TMath::Tan(ThetaF1) * TMath::Sin(PhiF) +
              CH4GapWidth * TMath::Tan(ThetaF2) * TMath::Sin(PhiF);
        ...
        if (TMath::Sqrt((Xf - X0) * (Xf - X0) + (Yf - Y0) * (Yf - Y0)) >
            TMath::Sqrt((Xcen[n1] - Xpi - X0) * (Xcen[n1] - Xpi - X0) +
                        (Ycen[n1] - Ypi - Y0) * (Ycen[n1] - Ypi - Y0)))
      ...
      TVector3 vP((TMath::Sin(ThetaP)) * (TMath::Cos(PhiP)),
                  (TMath::Sin(ThetaP)) * (TMath::Sin(PhiP)),
                  (TMath::Cos(ThetaP)));
      TVector3 vz(0., 0., 1.);

      TVector3 v1 = vP.Cross(vz);

      TVector3 vF((TMath::Sin(ThetaF)) * (TMath::Cos(PhiF)),
                  (TMath::Sin(ThetaF)) * (TMath::Sin(PhiF)),
                  (TMath::Cos(ThetaF)));
      ...
      if (DegPhiCherenkov[n1] < 0)
        DegPhiCherenkov[n1] += 360;
      ...
      // if(k1==2233) cout << " ThetaCherenkov " <<ThetaCherenkov[n1] <<"
      // PhiCherenkov = "<<DegPhiCherenkov[n1]<<"  event number = "<<k1<< endl;
    ...
    for (Int_t k2 = 0; k2 < 700; k2++) {
      ...
      for (Int_t p = 0; p < NumberOfClusters; p++) {
        ...
        if (ThetaCherenkov[p] > (0.001 * k2) &&
            ThetaCherenkov[p] < (0.001 * Hwidth + 0.001 * k2))
          NphotHough++;
      }
    ...
    for (Int_t p = 0; p < NumberOfClusters; p++) {
      if (ThetaCherenkov[p] > (0.001 * LocPos) &&
          ThetaCherenkov[p] < (Hwidth + 0.001 * LocPos)) {
  ...
  TCanvas *photonCanvas =
      new TCanvas("Photon Hit Map", "Photon Hit Map", 800, 800);
  TCanvas *photonCanvas2 =
      new TCanvas("Photon Hit Map2", "Photon Hit Map2", 800, 800);

  TCanvas *clusterCanvas =
      new TCanvas("Cluster Hit Map", "Cluster Hit Map", 800, 800);
  TCanvas *clusterCanvas2 =
      new TCanvas("Cluster Hit Map2", "Cluster Hit Map2", 800, 800);
  ...
  for (Int_t n2 = 0; n2 < NumberOfClusters; n2++) {
    ...
    if (ThetaCherenkov[n2] > 0.75)
      continue;

    Int_t bin = (Int_t)(ThetaCherenkov[n2] / (0.001)) + 1;

    // Printf("bin = %i, hTheta->GetBinContent(bin)=%f",bin,
    // hTheta->GetBinContent(bin));

    // if(hTheta->GetBinContent(bin)==0) bin=bin+1;

    Double_t weight =
        1 - fBackGround->Eval(ThetaCherenkov[n2]) / hTheta->GetBinContent(bin);
  ...
  TCanvas *ringCanvas =
      new TCanvas("Ring Cherenkov", "Ring Cherenkov", 800, 800);
  ringCanvas->cd();
  hThetaRing->Draw();
...

  {
    ...
    hTheta_cl->Draw();
    hTheta_cl->SetTitle(
        "a) Background Angle Distribution;angle [rad]; counts/1 mrad");
    ...
    hTheta2Log->SetTitle("b) LogY Photon Cherenkov Angle Distribution;angle "
                         "[rad]; counts/1 mrad");
    ...
    hTheta2_cl->Draw();
    hTheta2_cl->SetTitle(
        "c) Photon Cherenkov Angle Distribution;angle [rad]; counts/1 mrad");
    ...
    hThetawg_cl->SetTitle("d) (Weighted) Track Photon Cherenkov Angle "
                          "Distribution;angle [rad]; counts/1 mrad");
    hThetawg_cl->Draw();
  }
...
Double_t BackgroundFunc(Double_t *x, Double_t *par) {
  ...
  Double_t f =
      par[0] * TMath::Tan(TMath::ASin(1.2903 * TMath::Sin(xx))) *
