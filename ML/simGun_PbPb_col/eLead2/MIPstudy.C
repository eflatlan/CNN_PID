...
struct ShallowDigit {
  ...
  int getSourceId () const  {return mSourceId;}
    ShallowDigit(uint16_t q, uint8_t x, uint8_t y, Int_t trackId, Int_t
particlePdg) : mQ(q), mX(x), mY(y), trackId(mTrackId),particlePdg(mParticlePdg)
{}
}; */
...
trackIndex fra track int trackId = -1; bool isMip = false;
    ...
    ClusterCandidate(int ch, double x, double y, double q, double chi2,
                     double xe, double ye, /*std::vector<Topology>*
topologyVector,* /, int pdg, std::vector<std::pair<int,int>>*
candidateStatusVector) : mCh(ch), mX(x), mY(y), mQ(q), mChi2(chi2), mXe(xe),
mYe(ye),
          /*mTopologyVector(topologyVector),* / mPDG(pdg),
mCandidateStatusVector(candidateStatusVector) {}
...
void MIPstudy() {
  ...
  auto myTree = std::make_unique<TTree>(
      "myTree", "Tree to store clusters, trackInfo, and mcPDG");
  ...
  TTree *tCluster = HmpidDataReader::initializeClusterTree(
       ...
       i++) // for(const auto& clusters : clustersVector) // "events loop"
  {
    ...
    // Printf("Checking trigger number %d Range Clu = %d :: %d", i, firstEntry,
    // lastEntry);
    ...
    if(eventNumberLast != eventNumber1) {
        Printf("eventNumberLast%d != eventNumber1%d", eventNumberLast,
    eventNumber1); Printf("eventNumberLast1%d",eventNumberLast1); } // TODO:
    throw error? ef:
    */
...
std::cout << " clu " << j << " eventNum " << clu.getEventNumber();
    ...
    std::vector<o2::dataformats::MatchInfoHMP> *tracksOneEvent =
        HmpidDataReader::readMatch(tMatch, matchArr, i, startIndexTrack);

    // get MC tracks for given event from mc;
    // Printf("Reading vector<o2::MCTrack>* mcTracks for  eventNumber %d",
    // eventNumber1); std::vector<o2::MCTrack>* mcTracks =
    // HmpidDataReader::readMC(mcArr, tMcTrack, eventNumber1);
    ...
    for (const auto &obj : *tracksOneEvent) {
      ...
      if (iCh < 0 || iCh > 6) {
        ...
        // sstd::cerr << "sortedTracks[iCh] " << iCh << " pushback" <<
        // std::endl;
      }
    }
    ...
    for (const auto &obj : oneEventClusters) {
      ...
      if (iCh >= 0 && iCh <= 6) {
        if (sortedTracks[iCh].size() > 0) {
          ...
          // obj.getClusterTopology();  // some info about digits associated w
          // cluster*/
          ...
          // std::cerr << "sortedTracks[iCh] " << iCh << " empty " <<
          // sortedTracks[iCh].size() << std::endl;
        }
    ...
    for (int i = 0; i < 7; i++) {
      ...
      for (const auto &track : sortedTracks[i]) {
        if (track.getMatchStatus()) {
          ...
          std::cout << "clusterBranch: " << cluSizeBranch << std::endl;
          std::cout << "cluChargeBranch: " << cluChargeBranch << std::endl;
          clusterBranch =
              const_cast<std::vector<o2::hmpid::ClusterCandidate> *>(
                  &clusterPerChamber);
          trackInfoBranch = const_cast<o2::dataformats::MatchInfoHMP *>(&track);
  ...
  std::cout << "mParticleEvents.writeH5(); " << std::endl;
  fOut->cd();

  myTree->Write();
  std::cout << "	myTree->Write(); " << std::endl;
  fOut->Close();

  return;
  std::cout << "fOut->Close(); " << std::endl;

  Long64_t nEntries = myTree->GetEntries();
  // for (Long64_t i = 0; i < nEntries; ++i) {
  // myTree->GetEntry(i);

  // The TBranches you are interested in (clusterBranch, trackInfoBranch,
  // mcPDGBranch) should now be filled with the data from the i-th entry.

  // Perform some checks or analysis using these variables.
  // For example, to print the size of clusterBranch (assuming it is a
  // std::vector)
  // std::cout << "clusterBranch: " << cluSizeBranch << std::endl;
  // std::cout << "cluChargeBranch: " << cluChargeBranch << std::endl;
...
void read_tree() {
  TCanvas *canvas1 =
      new TCanvas("canvas1", "Particle Distributions", 1200, 400);

  // Initialize histograms
  TH1F *momHistPion =
      new TH1F("momHistPion", ";Cherenkov Angle;Frequency", 50, 0.5, 0.728);
  TH1F *momHistKaon =
      new TH1F("momHistKaon", ";Cherenkov Angle;Frequency", 50, 0.5, 0.728);
  TH1F *momHistProton =
      new TH1F("momHistProton", ";Cherenkov Angle;Frequency", 50, 0.5, 0.728);
  ...
  TH1F mipSizeHistPion("mipSizeHistPion",
                       "MIP Size Distribution; Size; Frequency", 9, 0, 9);
  TH1F mipQHistPion("mipQHistPion",
                    "MIP Charge Distribution; Charge; Frequency", 50, 0, 8000);
  TH2F mip3DHistPion("mip3DHistPion", "MIP 3D Distribution; Size; Charge", 9, 0,
                     9, 50, 0, 8000);

  float refIndexBranchP, xRadBranchP, yRadBranchP, xMipBranchP, yMipBranch,
      thBranchP, phBranchP;
  int mcPDGBranchP, cluSizeBranchP, cluChargeBranchP;

  treePion->SetBranchAddress("cluCharge", &cluChargeBranchP);
  treePion->SetBranchAddress("cluSize", &cluSizeBranchP);
  treePion->SetBranchAddress("refIndex", &refIndexBranchP);
  treePion->SetBranchAddress("xRad", &xRadBranchP);
  treePion->SetBranchAddress("yRad", &yRadBranchP);
  treePion->SetBranchAddress("xMip", &xMipBranchP);
  treePion->SetBranchAddress("yMip", &yMipBranch);
  treePion->SetBranchAddress("th", &thBranchP);
  treePion->SetBranchAddress("ph", &phBranchP);
  treePion->SetBranchAddress("mcPDG", &mcPDGBranchP);

  float refIndexBranchPr, xRadBranchPr, yRadBranchPr, xMipBranchPr,
      yMipBranchPr, thBranchPr, phBranchPr;
  int mcPDGBranchPr, cluSizeBranchPr, cluChargeBranchPr;

  treeProton->SetBranchAddress("cluCharge", &cluChargeBranchPr);
  treeProton->SetBranchAddress("cluSize", &cluSizeBranchPr);
  treeProton->SetBranchAddress("refIndex", &refIndexBranchPr);
  treeProton->SetBranchAddress("xRad", &xRadBranchPr);
  treeProton->SetBranchAddress("yRad", &yRadBranchPr);
  treeProton->SetBranchAddress("xMip", &xMipBranchPr);
  treeProton->SetBranchAddress("yMip", &yMipBranchPr);
  treeProton->SetBranchAddress("th", &thBranchPr);
  treeProton->SetBranchAddress("ph", &phBranchPr);
  treeProton->SetBranchAddress("mcPDG", &mcPDGBranchPr);

  TH1F mipSizeHistProton("mipSizeHistProton",
                         "MIP Size Distribution; Size; Frequency", 9, 0, 9);
  TH1F mipQHistProton("mipQHistProton",
                      "MIP Charge Distribution; Charge; Frequency", 50, 0,
                      8000);
  TH2F mip3DHistProton("mip3DHistProton", "MIP 3D Distribution; Size; Charge",
                       9, 0, 9, 50, 0, 8000);

  // Assume that all trees have the same number of entries
  Long64_t pionEntries = treePion->GetEntries();
  Long64_t kaonEntries = treeKaon->GetEntries();
  Long64_t protonEntries = treeProton->GetEntries();

  if (!fPion || !fKaon || !fProton) {
    std::cerr << "Error opening pion file!" << std::endl;
    return;
  }
  if (!treePion || !treeKaon || !treeProton) {
    std::cerr << "Error opening pion file!" << std::endl;
    return;
  }
  for (Long64_t i = 0; i < pionEntries; ++i) {
    // Fill histograms based on momentum range

    if (i < pionEntries) {
      treePion->GetEntry(i);
      // if (pPion > 2.8 && pPion < 2.85 && ckovPion > 0.66 && 0.66 + 0.048 >
      // ckovPion) {
      if (mcPDGBranchP == 211 || mcPDGBranchP == -211) {
        // fill histogram...;:
        Printf(" cluSizeBranchP %d charge %d", cluSizeBranchP,
               cluChargeBranchP);
        mipSizeHistPion.Fill(cluSizeBranchP);
        mipQHistPion.Fill(cluChargeBranchP);
        mip3DHistPion.Fill(cluSizeBranchP, cluChargeBranchP);
    ...
    if (i < protonEntries) {
      ...
      // if (pProton > 2.8 && pProton < 2.85 && ckovProton > 0.59 && 0.645 >
      // ckovProton) {
      if (mcPDGBranchPr == 2212 || mcPDGBranchPr == -2212) {
        // fill histogram...;:
        mipSizeHistProton.Fill(cluSizeBranchPr);
        mipQHistProton.Fill(cluChargeBranchPr);
        mip3DHistProton.Fill(cluSizeBranchPr, cluChargeBranchPr);
  ...
  TCanvas canvas2("canvas2", "MIP Charge Distributions", 1200, 400);
  canvas2.Divide(2);
  canvas2.cd(1);
  mipQHistPion.Draw();
  canvas2.cd(2);
  ...
  TCanvas canvas3("canvas3", "MIP Size Distributions", 1200, 400);
  canvas3.Divide(2);
  canvas3.cd(1);
  mipSizeHistPion.Draw();
  canvas3.cd(2);
  mipSizeHistProton.Draw();

  TCanvas canvas4("canvas4", "MIP 3D Distribution - 2D View", 1200, 400);
  canvas4.Divide(2);
  canvas4.cd(1);
  mip3DHistPion.Draw("COLZ");
  canvas4.cd(2);
  mip3DHistProton.Draw("COLZ");

  TCanvas canvas5("canvas5", "MIP 3D Distribution - 3D View", 1200, 400);
  canvas5.Divide(2);
  canvas5.cd(1);
  mip3DHistPion.Draw("SURF");
  canvas5.cd(2);
  mip3DHistProton.Draw("SURF");
  ...
  TLatex lPion;
  // lPion.DrawLatex(0.45, momHistPion->GetMaximum() * 0.9, Form("#entries =
  // %lld, #mu = %.3f, #sigma = %.3f", momHistPion->GetEntries(),
  // fitPion->GetParameter(1), fitPion->GetParameter(2)));
  ...
  TLatex lKaon;
  // lKaon.DrawLatex(0.45, momHistKaon->GetMaximum() * 0.9, Form("#entries =
  // %lld, #mu = %.3f, #sigma = %.3f", momHistKaon->GetEntries(),
  // fitKaon->GetParameter(1), fitKaon->GetParameter(2)));
  ...
  TLatex lProton;
  // lProton.DrawLatex(0.45, momHistProton->GetMaximum() * 0.9, Form("#entries =
  // %lld, #mu = %.3f, #sigma = %.3f", momHistProton->GetEntries(),
  // fitProton->GetParameter(1), fitProton->GetParameter(2)));
  ...
  canvas2.Update();
  canvas2.SaveAs("MIPChargePlots.png");
  canvas2.SaveAs("MIPChargePlots.jpg");
  canvas2.SaveAs("MIPChargePlots.pdf");
  canvas2.SaveAs("MIPChargePlots.eps");

  canvas3.Update();
  canvas3.SaveAs("MIPSizePlots.png");
  canvas3.SaveAs("MIPSizePlots.jpg");
  canvas3.SaveAs("MIPSizePlots.pdf");
  canvas3.SaveAs("MIPSizePlots.eps");

  canvas4.Update();
  canvas4.SaveAs("MIP3D2DViewPlots.png");
  canvas4.SaveAs("MIP3D2DViewPlots.jpg");
  canvas4.SaveAs("MIP3D2DViewPlots.pdf");
  canvas4.SaveAs("MIP3D2DViewPlots.eps");

  canvas5.Update();
  canvas5.SaveAs("MIP3D3DViewPlots.png");
  canvas5.SaveAs("MIP3D3DViewPlots.jpg");
  canvas5.SaveAs("MIP3D3DViewPlots.pdf");
  canvas5.SaveAs("MIP3D3DViewPlots.eps");
}
...
void evaluateClusterTrack(
  ...
  const auto nF =
      track.getRefIndex(); // ef: aon it only gets the mean value ; TODO: in
                           // MatchHMP.cxx get calibration value
  ...
  // double radParams[7] = {xRad,yRad,L,thetaP, phiP, randomValue.momentum,
  // randomValue.mass};
  ...
  // clusterPerChamber by reference, add element to vector {trackNumber,
  // bitHadronStatus}
