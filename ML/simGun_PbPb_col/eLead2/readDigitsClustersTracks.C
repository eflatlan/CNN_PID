...
void readDigits(char *filename, int nEvent) {
  ...
  TH2F *hParticlePdgElectron[7], *hParticlePdgPion[7], *hParticlePdgKaon[7],
      *hParticlePdgProton[7], *hParticlePdgPhoton[7], *hParticlePdgPhoton2[7];

  TH2F *hParticlePdgElectron1[7], *hParticlePdgPion1[7], *hParticlePdgKaon1[7],
      *hParticlePdgProton1[7], *hParticlePdgPhoton1[7],
      *hParticlePdgPhoton21[7];

  // 11 111 321 2212 22/50000050
  for (int i = 0; i < 7; i++) {
    // Electron histograms

    hParticlePdgElectron[i] = new TH2F(
        Form("Electron_ParticlePdg_Chamber%i", i),
        Form("Electron PDG Map Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    // Pion histograms
    hParticlePdgPion[i] =
        new TH2F(Form("Pion_ParticlePdg_Chamber%i", i),
                 Form("Pion PDG Map Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    // Kaon histograms
    hParticlePdgKaon[i] =
        new TH2F(Form("Kaon_ParticlePdg_Chamber%i", i),
                 Form("Kaon PDG Map Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    // Proton histograms
    hParticlePdgProton[i] =
        new TH2F(Form("Proton_ParticlePdg_Chamber%i", i),
                 Form("Proton PDG Map Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    // Photon histograms
    hParticlePdgPhoton[i] =
        new TH2F(Form("Photon_ParticlePdg_Chamber%i", i),
                 Form("Photon PDG Map Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    hParticlePdgPhoton2[i] = new TH2F(Form("Photon2_ParticlePdg_Chamber%i", i),
                                      Form("Photon2 PDG Map Chamber%i", i), 160,
                                      0, 159, 144, 0, 143);
    ...
    hParticlePdgElectron1[i] =
        new TH2F(Form("Electron_Particle  Pdg_Chamber%i", i),
                 Form("Electron PDG Map  1 Event Chamber%i", i), 160, 0, 159,
                 144, 0, 143);
    ...
    // Pion histograms
    hParticlePdgPion1[i] = new TH2F(
        Form("Pion_Particle 1 Event Pdg_Chamber%i", i),
        Form("Pion PDG Map 1 Event Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    // Kaon histograms
    hParticlePdgKaon1[i] =
        new TH2F(Form("Kaon_ParticlePdg 1 Event _Chamber%i", i),
                 Form("Kaon PDG Map Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    // Proton histograms
    hParticlePdgProton1[i] = new TH2F(
        Form("Proton_ParticlePdg_Chamber%i", i),
        Form("Proton PDG Map 1 Event Chamber%i", i), 160, 0, 159, 144, 0, 143);
    hParticlePdgProton[i]->SetXTitle("pad X");
    ...
    // Photon histograms
    hParticlePdgPhoton1[i] = new TH2F(
        Form("Photon_Particle Pdg 1 Event Chamber%i", i),
        Form("Photon PDG Map 1 Event Chamber%i", i), 160, 0, 159, 144, 0, 143);
    ...
    hParticlePdgPhoton21[i] = new TH2F(
        Form("Photon2_ParticlePdg 1 Event Chamber%i", i),
        Form("Photon2 PDG Map 1 Event Chamber%i", i), 160, 0, 159, 144, 0, 143);
  ...
  for (int i = 0; i < 7; i++) {
    hParticlePdgMapAbove[i] =
        new TH2F(Form("Digits ParticlePdg Map Above 10000 chamber%i", i),
                 Form("Digits ParticlePdg Map Above 10000 chamber%i", i), 160,
                 0, 159, 144, 0, 143);
    hParticlePdgMapAbove[i]->SetXTitle("pad X");
    hParticlePdgMapAbove[i]->SetYTitle("pad Y");
    hParticlePdgMapAbove[i]->SetMarkerColor(kRed);

    hParticlePdgMapBelow[i] =
        new TH2F(Form("Digits ParticlePdg Map Below 10000 chamber%i", i),
                 Form("Digits ParticlePdg Map Below 10000 chamber%i", i), 160,
                 0, 159, 144, 0, 143);
  ...
  for (int i = 0; i < 7; i++) {
    hParticleChargeOneEvent[i] =
        new TH2F(Form("Charge per event map%i", i),
                 Form("Charge per event map%i", i), 160, 0, 159, 144, 0, 143);
  ...
  for (unsigned int j = 0; j < digits->size(); j++) {
    ...
    if (pDig->mParticlePdg > 10000) {
      ...
      // std::cout << j << " Photon " << pDig->mParticlePdg << " Ch " << module
      // << std::endl;
    ...
    for (const auto &pdg : particlePDGs) {
      if (pdg == pDig->mParticlePdg) {
        isInPdg = true;
      }
    }
    if (!isInPdg) {
      particlePDGs.push_back(pDig->mParticlePdg);
    }
  ...
  for (int i = 0; i < trigger->size(); i++) {
    ...
    for (int j = pTgr->getFirstEntry(); j <= pTgr->getLastEntry(); j++) {
      ...
      o2::hmpid::Digit::pad2Absolute(digit.getPadID(), &module, &padChX,
                                     &padChY);
...
TH1F *thcluMIPCharge[7], *thcluMIPSize[7], *thTrackMIPCharge[7],
    *thTrackMIPSize[7]; // add this
...
void process() {
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    int iCh = i;
    thcluMIPCharge[iCh] =
        new TH1F(Form("Clu Charge chamber%i", i),
                 Form("Clu Charge chamber%i", i), 2000, 100., 2100.); // init

    thcluMIPSize[iCh] = new TH1F(Form("MIP Size chamber%i", i),
                                 Form("MIP Size chamber%i", i), 10, 0., 10.);

    thTrackMIPCharge[iCh] =
        new TH1F(Form("Track MIP Charge chamber%i", i),
                 Form("Track MIP Charge chamber%i", i), 2000, 100., 4100.);
    // init
    thTrackMIPSize[iCh] =
        new TH1F(Form("Track MIP Size chamber%i", i),
                 Form("Track MIP Size chamber%i", i), 10, 0., 10.);

    // For thTrackMip
    thTrackMip[i] = new TH2F(Form("thTrackMip_%d", i),
                             Form("MIP Track %d; x-axis; y-axis", i), 160, 0,
                             159, 144, 0, 143);
    thTrackMip[i]->SetMarkerColor(kRed); // Example color
    thTrackMip[i]->SetMarkerStyle(2);    // Example color
    // For thTrackPc
    thTrackPc[i] = new TH2F(Form("thTrackPc_%d", i),
                            Form("PC Track %d; x-axis; y-axis", i), 160, 0, 159,
                            144, 0, 143);
    thTrackPc[i]->SetMarkerColor(kBlue - 3); // Example color
    thTrackPc[i]->SetMarkerStyle(2);         // Example color
    // For thTrackRad
    thTrackRad[i] = new TH2F(Form("thTrackRad_%d", i),
                             Form("Rad Track %d; x-axis; y-axis", i), 160, 0,
                             159, 144, 0, 143);
    ...
    // For hCluChargeMap
    hCluChargeMap[i] =
        new TH2F(Form("hCluChargeMap_%d", i),
                 Form("Cluster Charge Map %d; x-axis; y-axis", i), 160, 0, 159,
                 144, 0, 143);
  }
...
std::cout << j << " evNum " << clu.getEventNumber() << " |";
    ...
    std::vector<o2::dataformats::MatchInfoHMP> *tracksOneEvent =
        HmpidDataReader::readMatch(tMatch, matchArr, eventNumber1,
                                   startIndexTrack);
    ...
    for (const auto &obj : *tracksOneEvent) {
      ...
      std::cerr << " MIP Charge Track" << q << " Size  "
                << obj.getMipClusSize();
    ...
    for (int i = 0; i < 7; i++) {
      std::cout << "Length of sortedTracks vector " << i << ": "
                << sortedTracks[i].size() << std::endl;
    }
  ...
  TCanvas *cCluCharge =
      new TCanvas("cCluCharge", "Cluster Charge MAP", 800, 600);
  TCanvas *cTrackInfo =
      new TCanvas("cTrackInfo", "Track impact and mip", 800, 600);
...
void printMIP() {
  TCanvas *cthcluMIPCharge =
      new TCanvas("Cluster Charge", "Cluster Charge", 800, 600);
  TCanvas *cthcluMIPSize =
      new TCanvas("thcluMIPSize", "thcluMIPSize", 800, 600);
  TCanvas *cthTrackMIPCharge =
      new TCanvas("thTrackMIPCharge", "thTrackMIPCharge", 800, 600);
  TCanvas *cthTrackMIPSize =
      new TCanvas("thTrackMIPSize", "thTrackMIPSize", 800, 600);
