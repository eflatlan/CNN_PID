...
public:
  Populate(TVector2 trkPos, TVector3 trkDir, double _nF)
  ...
  bool checkRange2(const TVector2 &photonPos, const double &etaMax,
                   const double &etaMin, TVector2 &rPos) {
    ...
    // Printf("CheckRange : rMin %.3f < rPhoton %.3f, rMax %.3f", rMin, rPhoton,
    // rMax);
    return (rMin < rPhoton && rPhoton < rMax);
  }

  // find R at specific value of Phi to apply mass-hyp
  double getRatPhi(const TVector2 &photonPos, const double &eta,
                   const double &L, TVector2 &rPos) {
    ...
    const auto cosTheta =
        (TMath::Cos(eta) - sinThetaP * TMath::Cos(phi - phiP)) / cosThetaP;
    ...
    const auto b = (radThick - L) * tanTheta +
                   winThick * nF * sinTheta / TMath::Sqrt(nQ * nQ - nFSinSq) +
                   gapThick * nF * sinTheta / TMath::Sqrt(nG * nG - nFSinSq);

    const auto xDiff = (a * cosPhiP - b * cosPhi);
    const auto yDiff = (a * sinPhiP - b * sinPhi);
    const auto R = TMath::Sqrt(xDiff * xDiff + yDiff * yDiff);

    Printf("eta %.2f , Math::Cos(eta) %.2f | sinThetaP * TMath::Cos(phi-phiP) "
           "%.2f | cosThetaP %.2f",
           eta, TMath::Cos(eta), sinThetaP * TMath::Cos(phi - phiP), cosThetaP);
    ...
    Printf("getRatPhi : fPC: x %.2f y %.2f | Photon  x %.2f y %.2f | rPos x "
           "%.2f y %.2f",
           fPc.X(), fPc.Y(), photonPos.X(), photonPos.Y(), rPos.X(), rPos.Y());
  ...
  TVector2 traceForward(TVector3 &dirCkov, const double &L) const {
    ...
    if (thetaCer > TMath::ASin(1. / getRefIdx)) {
      Printf("populate : traceForward() INVOKED thetaCer > TMath::ASin(1. / "
             "getRefIdx))");
