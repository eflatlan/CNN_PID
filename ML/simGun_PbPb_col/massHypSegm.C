...
void populateRegions(Populate *populate,
                     std::vector<std::pair<double, double>> &vecArr, TH2F *map,
                     const double &eta, const double &l) {

  const int kN = vecArr.size();
  for (int i = 0; i < vecArr.size(); i++) {
    const auto &value =
        populate->tracePhot(eta, Double_t(TMath::TwoPi() * (i + 1) / kN), l);
    if (value.X() > 0 && value.X() < 156.0 && value.Y() > 0 &&
        value.Y() < 144) {
...
TH1D *phots = new TH1D("Photon Candidates",
                       "Photon Candidates;angle [rad]; counts/1 mrad",
                       nChannels, 0, kThetaMax);
TH1D *photsw =
    new TH1D("Photon Weights", "Photon Weights;angle [rad]; counts/1 mrad",
             nChannels, 0, kThetaMax);
TH1D *resultw =
    new TH1D("Sum of Weights in Window at Bin",
             "Sum of Weights in Window at bin;angle [rad]; counts/1 mrad",
             nChannels, 0, kThetaMax);
TH1F *hTheta = new TH1F("Background", "Background; angle [rad]; counts/1 mrad",
                        750, 0., 0.75);

TH1F *hThetaCh =
    new TH1F("Cherenkov Photons",
             "Cherenkov Photons; angle [rad]; counts/1 mrad", 750, 0., 0.75);
...
float /*std::array<TH1D*, 3>*/
houghResponse(std::vector<float> &photonCandidates, float fWindowWidth);

// make event number i:
std::vector<std::pair<double, double>> makeEvent(float occupancy,
...
const float refIndexQuartz =
    1.5875; // GetQuartzIndexOfRefraction(defaultPhotonEnergy);
...
float randomMomentum() { return 1 + 4 * rndInt->Gaus(0.5, 0.25); }

TH2F *tHistMass = new TH2F(
    "test", "test; Momentum (GeV/c); Cherenkov Angle, #theta_{ch} (rad)", 5000,
    0., 5., 800, 0., 0.8);

void testHyp() {
  TCanvas *tCkov = new TCanvas("ckov", "ckov", 800, 800);
  // TH2F *hClusterMap = new TH2F("Cluster Map", "Cluster Map; x [cm]; y
  // [cm]",1000,-10.,10.,1000,-10.,10.);
  rndInt->SetSeed(0);
...
const float ckovConstraintWhidth = 0.015; // 15 mrad per side for ckovangle
                                          // constraint from mass hypotheses
...
void saveDataInst();
/*std::shared_ptr<TFile>*/ void
saveParticleInfoToROOT(const std::vector<ParticleInfo> &particleVector);

double scaleThetaP;

void testRandomMomentum(int numObjects = 10, float thetaTrackInclination = 0,
                        double occupancy = 0.03, double _scaleThetaP = 1) {
  ...
  for (auto &randomValue : randomObjects) {
    ...
    const auto filledBins =
        makeEvent(occupancy, randomValue, particle, eventCnt);

    Printf("exit makeEvent made particle.Cand2 : \n  particleØØØ %zu",
           particle.candidatesCombined.size());
    ...
    // TODO : hSignalAndNoiseMap just placeholder, make instead a TH2F from the
    // filledBins?

    /*
    const auto& map =  new TH2F("Signal and Noise2 ", "Signal and Noise2 ; x
    [cm]; y [cm]",160,0.,159.,144,0,143); */

    // make sure the momentum is valid for the given particle (e.g., no - in the
    // square-root in calcCkovFromMass and acos [-1..1])
    if (particle.ckov == 0) {
      continue;
    }
    i++;

    // ParticleInfo Has to be extended to also contain xRad, yP, (xMIP, yMIP?),
    // thetaP, phiP

    particleVector.emplace_back(particle);

    Printf("CkovAngle %f Mass %f RefIndex %f Momentum %f | Num Entries in Map "
           ": %zu",
           particle.ckov, particle.mass, particle.refractiveIndex,
           particle.momentum /*, particle.filledBins.size()*/);
  ...
  for (const auto &pV : particleVector) {
    ...
    Printf(" pionCandidates size %zu  protonCandidates %zu kaonCandidates %zu",
           pC.size(), prC.size(), kC.size());
...
std::vector<std::pair<double, double>> makeEvent(float occupancy,
  ...
  const auto &ckov = calcCkovFromMass(
      randomValue.momentum, randomValue.refractiveIndex,
      randomValue.mass); //  calcCkovFromMass(momentum, n, mass)

  // theoretical ckov angles :

  // TODO: fjern denne ighen
  // auto momentum = 1.5; //
  // const auto& ckovHyps2 = calcCherenkovHyp(momentum,
  // randomValue.refractiveIndex);

  // funker ikke segm på p = .5? / jo, kun at momentum settes til annen verdi
  // enn rV.momentum
  const auto &ckovHyps =
      calcCherenkovHyp(randomValue.momentum, randomValue.refractiveIndex);

  if (particle.energy < 5)
    particle.energy = 6.75;
  auto ckovAngle = ckov;

  Int_t NumberOfEvents = 1;
  Int_t NumberOfClusters = 13;
  float Hwidth = 15.;
  // testRandomMomentum();
  ...
  QuartzIndexOfRefraction =
      1.5787; // GetQuartzIndexOfRefraction(particle.energy);
  ...
  // TH2F *hSignalAndNoiseMap = new TH2F("Signal and Noise ", "Signal and Noise
  // ; x [cm]; y [cm]",1000,-25.,25.,1000,-25.,25.);
  ...
  double phiP =
      TMath::Pi() / 4; // static_cast<float>((3.14159)*(1-2*gRandom->Rndm(1)));
  ...
  double thetaP =
      ...
      getThetaP(
          randomValue
              .momentum); // = static_cast<float>(0.1*(1-2*gRandom->Rndm(1)));
  ...
  double xRad =
      1 * static_cast<float>((160 - diff) * (1 * gRandom->Rndm()) + diff);
  double yRad =
      1 * static_cast<float>((144 - diff) * (1 * gRandom->Rndm()) + diff);

  auto winThick = 0.5, radThick = 1.5;
  int gapThick = 8;
  ...
  double radParams[7] = {
      xRad, yRad, L, thetaP, phiP, randomValue.momentum, randomValue.mass};
  ...
  const TVector2 &posMaxProton =
      populate->tracePhot(ckovTools.getMaxCkovProton(), 0, lMax);
  /*
  for(double t = 0; t < 1; t+= 0.1) {

          const auto& valueL = populate->tracePhot(.7, t, L);
          const auto& valueR = populate->tracePhot(.7, -t, L);
           Printf("valueL %.2f, valueR %.2f --> rL %.2f , rR %.2f",
  (impPC-valueL).Mod(), (impPC-valueR).Mod());
  ...
  Printf("bgstudy segment : phiP %f thetaP %f xRad %f yP %f ", phiP, thetaP,
         xRad, yRad);

  Printf(" makeEvent%d : enter  numberOfCkovPhotons loop", eventCnt);
  Printf(" makeEvent%d : numberOfCkovPhotons %d", eventCnt,
         numberOfCkovPhotons);
  ...
  Printf(" makeEvent%d : ckovHyps = <%.3f, %.3f> | <%.3f, %.3f> | <%.3f, %.3f>",
  ...
  // Printf("		sizes = kN %d | %zu %zu %zu", kN, maxPionVec.size(),
  // maxKaonVec.size(), maxProtonVec.size());
  ...
  for (photonCount = 0; photonCount < numberOfCkovPhotons; photonCount++) {
    ...
    float etaC =
        rnd->Gaus(ckovAngle, 0.008); // random CkovAngle, with 0.012 std-dev

    float phiL = static_cast<float>((TMath::Pi()) * (1 - 2 * gRandom->Rndm(1)));
    // angle around ckov Cone of photon

    TVector2 phot = populate->tracePhot(
        etaC, phiL, L); // tracePhot(double ckovThe, double ckovPhi)
    // Printf("made photon using populate->tracePhot | L = %.3f, etaC = %.4f,
    // ckovAngle = %.4f | x %.3f, y %.3f ", L, etaC, ckovAngle, phot.X(),
    // phot.Y());
    ...
    float diff;
    if (phiL < 0)
      diff = TMath::TwoPi() - (vec.Phi() - phiL);
    else
      diff = vec.Phi() - phiL;
    // Printf("phiL %.4f | phiRingAngle  %.4f | diff %.4f | phiP %.4f ", phiL,
    // vec.Phi(), diff,  phiP);
    ...
    // {ckovPhotonCoordinates.first,ckovPhotonCoordinates.second , etaC};
    ...
    // Printf(" makeEvent : ckovTools.makeCkovPhoton returned x %f y%f",
    // ckovPhotonCoordinates.first + xRad, ckovPhotonCoordinates.second + yRad);
    // hSignalAndNoiseMap->Fill(phot.X(), phot.Y());
  }

  Printf(" makeEvent%d : Number of photons created :  %d ", eventCnt,
         photonCount);
  ...
  const auto photonCandidatesCoords = ckovTools.segment(
      cherenkovPhotons, pionCandidates, kaonCandidates, protonCandidates,
      arrayInfo, candCombined, temp); // temp --> mapBins

  for (const auto &c : candCombined) {
    Printf("segment : candCombined x %.2f y %.2f CS %d", c.x, c.y,
           c.candStatus);
  }

  Printf(" makeEvent%d : num photonCandidatesCoords %zu", eventCnt,
         photonCandidatesCoords.size());
  ...
  Printf("makeEvent%d() exit const auto& photons : photonCandidatesCoords",
         eventCnt);

  /*
  TCanvas *thSignalAndNoiseMap = new
  TCanvas("hSignalAndNoiseMap","hSignalAndNoiseMap",800,800);
  ...
  */
  ...
  gPad->Update();*/
  int cnt = 0;
  for (auto &b : mapBins) { /*Printf("xval %f", b.x);*/
    cnt++;
  }
  ...
  for (const auto &c : particle.candidatesCombined) {
    Printf("segment :  particle.candidatesCombined x %.2f y %.2f CS %d", c.x,
           c.y, c.candStatus);
  }
...
void saveParticleInfoToROOT2(const std::vector<ParticleInfo> &particleVector) {
  ...
  for (const auto &particle : particleVector) {
    // Delete the old object and create a new one with the current particle's
    // values
    delete particleInfo;
...
void saveHD5(const std::vector<ParticleInfo> &particleVectorIn) {
  // Convert TH2F* maps to 2D array and save ParticleInfo structs to an HDF5
  // file.
  ...
  for (const auto &particle : particleVectorIn) {
    ...
    std::cout << "  ckov: " << particle.ckov << "\n";
                                        Printf("  %d %d %d %d",aInfo[0],
    aInfo[1], aInfo[2], aInfo[3]);*/
  }
