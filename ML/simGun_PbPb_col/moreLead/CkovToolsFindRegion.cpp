...
%f",  x, xMaxPhiPi, x , xMaxPhi0 , y , yMaxPhiPi , y , yMaxPhi0);*/
      ...
      if (rPhoton < rMax && cStatus == 0) {
        ...
        if (getProtonStatus() and getKaonStatus() and getPionStatus()) {
          ...
          if (isMinProtonOk) {
            ...
            if (isMaxPionOk) {
              ...
              isMinPionOk = populate2Ptr->checkUnder(
                  posPhoton, rPhoton, phiPhoton, arrMinPion, getMinCkovPion(),
                  "Pion", shutDownOnOpen);
        ...
        // if Proton not is possible because momentum-threshold is not exceeded:
          ...
          isMinKaonOk = populate2Ptr->checkUnder(posPhoton, rPhoton, phiPhoton,
                                                 arrMinKaon, getMinCkovKaon(),
                                                 "Kaon", shutDownOnOpen);
        ...
        // not exceeded:
          ...
          if (isMaxPionOk) {
            ...
            bool shutDownOnOpen = false;
            bool isMinPionOk = populate2Ptr->checkUnder(
                posPhoton, rPhoton, phiPhoton, arrMinPion, getMinCkovPion(),
                "Pion", shutDownOnOpen);
            if (shutDownOnOpen) {
              errorPos.Set(posPhoton);
