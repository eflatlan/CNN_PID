...
auto phots = new TH1D("Photon Candidates",
                      "Photon Candidates;angle [rad]; counts/1 mrad", nChannels,
                      0, kThetaMax);
auto photsw =
...
TH2F *backgroundStudy(std::vector<Bin> &mapBins, float ckovActual = 0.5,
                      float occupancy = 0.01, float thetaTrack = 0);
...
void testRandomMomentum(int numObjects = 10, float thetaTrackInclination = 0) {
  ...
  for (const auto &randomValue : randomObjects) {
    ...
    const auto &map = backgroundStudy(
        mapBins, ckov, 0.001,
        thetaTrackInclination); // cherenkov angle mean / occupancy / theta
                                // track inclination (perpendicular =)
    ...
    Printf("CkovAngle %f Mass %f RefIndex %f Momentum %f | Num Entries in Map "
           ": %d",
           ckov, particle.mass, particle.refractiveIndex, particle.momentum,
           particle.filledBins.size());
...
TH2F *backgroundStudy(std::vector<Bin> &mapBins, float ckovActual = 0.5,
  ...
  TH2F *hSignalAndNoiseMap =
      new TH2F("Signal and Noise ", "Signal and Noise ; x [cm]; y [cm]", 160,
               0., 159., 144, 0, 143);

  float mapArray[40][40]{};

  float Deltax =
      (RadiatorWidth + QuartzWindowWidth + CH4GapWidth - EmissionLenght) *
      TMath::Tan(ThetaP) * TMath::Cos(PhiP);
  float Deltay =
      (RadiatorWidth + QuartzWindowWidth + CH4GapWidth - EmissionLenght) *
      TMath::Tan(ThetaP) * TMath::Sin(PhiP);

  Xpi = Xp - Deltax;
  Ypi = Yp - Deltay;

  float ThetaCherenkov[100000] = {0x0}, PhiCherenkov[100000] = {0x0},
        DegPhiCherenkov[100000] = {0x0};
...
{
  float k =
      TMath::Sqrt(1 + 46.411 / (113.763556 - x) + 228.71 / (328.51563 - x));
  return k;
}
...
float BackgroundFunc(float *x, float *par) {
  ...
  float f = par[0] * TMath::Tan(TMath::ASin(1.2903 * TMath::Sin(xx))) *
            (1 + TMath::Tan(TMath::ASin(1.2903 * TMath::Sin(xx))) *
                     TMath::Tan(TMath::ASin(1.2903 * TMath::Sin(xx))) * 1.2903 *
                     TMath::Cos(xx) / cos(asin(1.2903 * TMath::Sin(xx))));
...
float getRadiusFromCkov(float ckovAngle) {
  ...
  float sin_qz =
      static_cast<float>(sin_ckov * (refIndexFreon / refIndexQuartz));
  float sin_theta0 =
      static_cast<float>(sin_qz * (refIndexQuartz / refIndexCH4));
...
*/
  ...
  for (const auto &angle : photonCandidates) { // photon cadidates loop
    ...
    if (true) {
      ...
      float areaLow =
          0.5 * TMath::Pi() * TMath::Sq(rLow); // calcRingGeom(lowerlimit, 2);

      float rHigh = getRadiusFromCkov(upperlimit);
      float areaHigh =
          0.5 * TMath::Pi() * TMath::Sq(rHigh); // calcRingGeom(lowerlimit, 2);
  ...
  // photsw
  Printf("Entries : resultw %f photsw %f phots %f", resultw->GetEntries(),
         photsw->GetEntries(), phots->GetEntries());
  Printf("Max resultw %f photsw %f phots %f", resultw->GetMaximum(10000.),
         photsw->GetMaximum(10000.), phots->GetMaximum(10000.));
  Printf("Min resultw %f photsw %f phots %f", resultw->GetMinimum(-1.),
         photsw->GetMinimum(-1.), phots->GetMinimum(-1.));

  // ef: not this method, raw-pointers should not be used with
  // new/delete-keywords
  ...
  float ckovTrack = static_cast<float>(
      locMax * fDTheta + 0.5 * fDTheta); // final most probable track theta ckov
  ...
  TLine *l3 = new TLine(ckovTrack - (fWindowWidth / 2) * 0.001, 0,
                        ckovTrack - (fWindowWidth / 2) * 0.001, up);
  TLine *l4 = new TLine(ckovTrack + (fWindowWidth / 2) * 0.001, 0,
                        ckovTrack + (fWindowWidth / 2) * 0.001, up);
  TLine *l5 = new TLine(ckovTrack - (fWindowWidth / 2) * 0.001, up,
                        ckovTrack + (fWindowWidth / 2) * 0.001, up);
  ...
  TBox *box = new TBox(
      /*ckovTrack*/ ckovTrack - nCorrBand * 0.001, 0.,
      /*ckovTrack*/ ckovTrack + nCorrBand * 0.001,
      phots->GetBinContent(ckovTrack * 1000 /*photsw->GetMaximumBin()*/));
  ...
  auto l3c = static_cast<TLine *>(l3->Clone());
  auto l4c = static_cast<TLine *>(l4->Clone());
  auto l5c = static_cast<TLine *>(l5->Clone());
  l5c->SetY1(up_);
  l3c->SetY2(up_);
  l4c->SetY2(up_);
  l5c->SetY2(up_);
  ...
  TLine *l = new TLine(/*ckovTrack*/ ckovTrack, 0, /*ckovTrack*/ ckovTrack,
                       resultw->GetBinContent(locMax) * 2);

  // TLine* l = new TLine(resultw->GetMaximumBin(), 0.,
  // resultw->GetMaximumBin(),
  // resultw->GetBinContent(resultw->GetMaximumBin()));
...
/*
// save TH2F* in own TTree since it caused segmentation fault when writing to
same TTree as the other elements std::shared_ptr<TFile>
saveParticleInfoToROOT(const std::vector<ParticleInfo>& particleVector) {
