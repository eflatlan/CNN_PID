...
private:
  ...
  TVector3 fTrkDir; // track position in LORS at RAD // setMagThetaPhi(1,
                    // thetaP, phiP)
...
public:
  Populate2(TVector2 trkPos, TVector3 trkDir, double _nF)
      : fTrkPos(trkPos), fTrkDir(trkDir), nF(_nF) {
    ...
    */
    ...
    Printf("Track dir at RAD : theta %.3f phi %.3f ", trkDir.Theta(),
           trkDir.Phi());
  }
  ...
  TVector2 tracePhot(const double &ckovThe, const double &ckovPhi,
  ...
  TVector2 traceForward(TVector3 &dirCkov, const double &L) const {
    ...
    // the ckov photon can be above this value, just not the track cherenkov
    // value?

    // its the dispersion of chromacity that makes the normal distribution
    // around the track Ckov value?
    if (thetaCer > TMath::ASin(1. / getRefIdx)) {
      LOGP(debug,
           "populate2: traceForward() INVOKED thetaCer {} > TMath::ASin(1. / "
           "getRefIdx{}))",
           getRefIdx, thetaCer);
    ...
    propagate(dirCkov, posCkov,
              -0.5 * winThick); // TODO giacomo spm :er ikke dette også feil!
  ...
  void lors2Trs(const TVector3 &dirCkov, double &thetaCer,
                double &phiCer) const {
    TRotation mtheta;
  ...
  void trs2Lors(const TVector3 &dirCkov, double &thetaCer,
                double &phiCer) const {
    ...
    Printf("	new : phi %.3f, theta %.3f", dirCkovLORS2.Phi(),
    dirCkovLORS2.Theta());*/
  }
  ...
  void setTrackPos(const TVector2 &fTrkIn) { fTrkPos = fTrkIn; }

  TVector2 getTrackPos() const { return fTrkPos; }
  ...
  void setPcImp(const TVector2 &fPcImpIn) { fPc = fPcImpIn; }

  TVector2 getPcImp() const { return fPc; }

  // if getAbove == true; check if posPhoton is outside of radius
  // bool checkCond(const TVector2& posPhoton, const double& rPhoton, const
  // double& phiPhoton, bool getAbove, vecArray3& vec, const double& etaCkov) {
  bool checkCond(const TVector2 &posPhoton, const double &rPhoton,
                 const double &phiPhoton, bool getAbove, vecArray4 &vec,
                 const double &etaCkov) {
    ...
    Printf("\n\n Enter checkCond() \n initValue = %d \n  vec[initValue-1][1] = "
           "%.3f || phi2 = vec[initValue][1] =  %.3f |  vec[initValue+1][1] = "
           "%.3f;",
           initValue, vec[initValue - 1][1], vec[initValue][1],
           vec[initValue + 1][1]);
    ...
    while (/*phi2 < phiPhoton*/ true) {
      ...
      Printf("	while(phi12< phiPhoton) {  || phi2 %.4f, prev %.4f phiPhoton "
             "%.4f, iCnt %d, initValue + iCnt = %d",
             phi2, prev, phiPhoton, iCnt, initValue + iCnt);
      ...
      if (phi2 > phiPhoton && prev < phiPhoton && prev != 0) {
        Printf("\n phi2 %.6f > phiPhoton %.6f && prev %.6f < phiPhoton %.6f: "
               "exit!\n",
    ...
    Printf("	checkCond() -->  kN %d | initValue %d | phiPhoton %.6f, phiP "
           "%.3f, phiC %.3f, phi2 %.6f, phi1 %.3f,",
           kN, initValue, phiPhoton, phiP, phiC, phi2, phi1);

    // Printf("phiPhoton %.2f| vec[initValue + iCnt - 1] %.2f,  vec[initValue +
    // iCnt] %.2f, vec[initValue + iCnt + 1] %.2f", phiPhoton, vec[initValue +
    // iCnt-1][1],  vec[initValue + iCnt][1], vec[initValue + iCnt+ 1][1]);
    ...
    Printf("	getAbove == %d | rPhoton = %.2f, rMax = %.2f, rMin = %.2f",
           getAbove, rPhoton, rMax, rMin);
    ...
    if (getAbove) {
      while (!decisionTaken) {
        Printf(
            "	getAbove == %d  | rPhoton = %.2f, rMax = %.2f, rMin = %.2f",
            getAbove, rPhoton, rMax, rMin);

        Printf("	phiMax = %.4f, phiLmax = %.4f, phiMin = %.4f, phiLmin "
               "= %.4f, etaCkov %.4f, phiPhoton %.4f",
               phiMax, phiLmax, phiMin, phiLmin, etaCkov, phiPhoton);
        if ((rPhoton > rMax)) {
          ...
          Printf("	checkCond() | rMin = %.2f , rMax = %.2f,  ", rMin,
                 rMax);
    ...
    Printf("exit checkCond() with condition = %d|  rMin = %.2f , rMax = %.2f, "
           "rPhoton = %.2f ",
           condition, rMin, rMax, rPhoton);

    Printf("	phiMin = %.4f <  phiPhoton %.4f <  phiMax = %.4f, ", phiMin,
           phiPhoton, phiMax);

    if (rMin > rMax) {
      Printf("NB!!!!!!!! exit checkCond() :: rMin %.2f > rMax %.2f ", rMin,
             rMax);
    }
  ...
  bool checkOver(const TVector2 &posPhoton, const double &rPhoton,
                 const double &phiPhoton, vecArray4 &vec, const double &etaCkov,
                 const char *hadronType) {
    // vec : contains phiL, phi, R w etaMin/etaMax for hadron species
    ...
    Printf("\n\n enter checkOver(%s) phiPhoton %.4f, phiP %.4f", hadronType,
           phiPhoton, phiP);

    // LOGP(info, "Some radius checks checkOver rPhoton {} : [sizeVec - 2]{}
    // [sizeVec - 1]{} [0]{} [1]{}", rPhoton, vec[sizeVec - 2][3],vec[sizeVec -
    // 1][3],vec[0][3],vec[1][3]);
    ...
    // %.3f || phi2 = vec[initValue][1] =  %.3f |  vec[initValue+1][1] = %.3f;",
    // hadronType,initValue, vec[sizeVec-1][1], vec[initValue][1],
    // vec[initValue+1][1]);

    // Printf(" phi::: %.3f | %.3f | %.3f | %.3f | %.3f | ;",
    // vec[sizeVec-1][1],vec[0][1], vec[1][1], vec[2][1], vec[3][1]);

    // Printf(" r::: %.3f | %.3f | %.3f | %.3f | %.3f | ;",
    // vec[sizeVec-1][2],vec[0][2], vec[1][2], vec[2][2], vec[3][2]);
    ...
    while (/*phi2 < phiPhoton*/ true) {
      ...
      // Printf("	while(phi12< phiPhoton) {  || phi2 %.4f, prev %.4f
      // phiPhoton %.4f, iCnt %d, initValue + iCnt = %d", phi2, prev, phiPhoton,
      // iCnt, initValue + iCnt);
    ...
    // vil det bli break Segmenetation fault siden vi prøver å finne element som
    // er out of range
    ...
    Printf("	checkOver(%s) -->  kN %d | initValue %d | phiPhoton %.6f, phiP "
           "%.3f, phiC %.3f, phi2 %.6f, phi1 %.3f,",
           hadronType, kN, initValue, phiPhoton, phiP, phiC, phi2, phi1);
    ...
    while (!decisionTaken) {
      ...
      Printf("	| rPhoton = %.2f, rMax = %.2f, rMin = %.2f", rPhoton, rMax,
             rMin);

      Printf("	phiMax = %.4f, phiLmax = %.4f, phiMin = %.4f, phiLmin = %.4f, "
             "etaCkov %.4f, phiPhoton %.4f",
             phiMax, phiLmax, phiMin, phiLmin, etaCkov, phiPhoton);
      ...
      if ((rPhoton < rMin + uncSquared)) {
        ...
        if (iCnt + 1 < sizeVec) {
          Printf("\n iCnt + 1 < sizeVec");
          Printf(" ??? r ==> -1 : %.3f 0 : %.3f 1: %.3f  ", vec[iCnt - 1][2],
                 vec[iCnt][2], vec[iCnt + 1][2]);
          Printf("??? phi ==> -1 : %.3f 0 : %.3f 1: %.3f  ", vec[iCnt - 1][1],
                 vec[iCnt][1], vec[iCnt + 1][1]);
          Printf("??? phiL ==> -1 : %.3f 0 : %.3f 1: %.3f  ", vec[iCnt - 1][0],
                 vec[iCnt][0], vec[iCnt + 1][0]);
        } else {
          Printf("\n iCnt + 1 == sizeVec");
          Printf(" ??? r ==> -1 : %.3f 0 : %.3f 1: %.3f  ", vec[iCnt - 1][2],
                 vec[iCnt][2], vec[0][2]);
    ...
    Printf("exit checkOver(%s) with condition = %d|  rMin = %.2f , rMax = "
           "%.2f, rPhoton = %.2f | L used for lim : %.2f, etaC = %.2f",
           hadronType, condition, rMin, rMax, rPhoton, lMin, etaCkov);

    Printf("	phiMin = %.4f <  phiPhoton %.4f <  phiMax = %.4f, ", phiMin,
           phiPhoton, phiMax);

    if (rMin > rMax) {
      Printf("NB!!!!!!!! exit checkOver(%s) :: rMin %.2f > rMax %.2f ",
             hadronType, rMin, rMax);
    }
  ...
  /*  vecArray4& vec {phiL, phiR, phiPC, r}
        ...
        // phiR : photon_phi i LORS system
                // phiPC : (photon - MIP).Phi(); --> vector phiL "on paper"
                        ...
                        const auto r = (max - trkPC2).Mod(); // trkPC2 : track
     impact @ PC
  ...
  bool checkUnder(const TVector2 &posPhoton, const double &rPhoton,
                  const double &phiPhoton, vecArray4 &vec,
                  const double &etaCkov, const char *hadronType,
                  bool &shutDownOnOpen) {
    // vec : contains phiL, phi, R w etaMin/etaMax for hadron species
    ...
    Printf("\n\n enter checkUnder(%s) phiPhoton %.4f, phiP %.4f", hadronType,
           phiPhoton, phiP);

    // LOGP(info, "Some radius checks checkUnder rPhoton {} : [sizeVec - 2]{}
    // [sizeVec - 1]{} [0]{} [1]{}", rPhoton,vec[sizeVec - 2][3],vec[sizeVec -
    // 1][3],vec[0][3],vec[1][3]);
    ...
    if (initValue + iCnt + 1 < sizeVec) {
      ...
      Printf("initValue %d + iCnt %d + 1 < sizeVec%d", initValue, iCnt,
             sizeVec);
      // double phiL1 = vec.at(index).at(0);
      phiL1 = vec[initValue + iCnt][0];
      phiL2 = vec[initValue + iCnt + 1][0];
    ...
     
      ...
      Printf("	checkUnder(%s) -->  phi2 == 0, setting back to 1st elem in "
             "arr");
      Printf("	Old vals : phi2 %.2f, phiL2 %.2f, r2 %.2f", phi2, phiL2, r2);
      phiL2 = vec[0][0];
      phi2 = vec[0][2];
    ...
    Printf("	checkUnder(%s) -->  kN %d | initValue %d | phiPhoton %.6f, "
           "phiP %.3f, phiC %.3f, phi2 %.6f, phi1 %.3f,",
           hadronType, kN, initValue, phiPhoton, phiP, phiC, phi2, phi1);
    ...
    while (!decisionTaken) {
      ...
      if ((rPhoton + uncSquared > rMax)) {
        ...
        condition = false;
                  ...
                  rPhoton + (uncSquared) < rMax)) {
        // iterate by splitting r1, r2 -> phi1 phi2

        splitPhi(phiMax, phiLmax, phiMin, phiLmin, etaCkov, phiPhoton); //
        rMax = getR(etaCkov, phiLmax, lMax);
        rMin = getR(etaCkov, phiLmin, lMax);
        Printf("	checkUnder(%s) | rMin = %.2f , rMax = %.2f,  ",
               hadronType, rMin, rMax);
      } else {
    ...
    Printf("exit checkUnder(%s) with condition = %d|  rMin = %.2f , rMax = "
           "%.2f, rPhoton = %.2f | L used for lim : %.2f, etaC = %.2f",
           hadronType, condition, rMin, rMax, rPhoton, lMax, etaCkov);

    Printf("	phiMin = %.4f <  phiPhoton %.4f <  phiMax = %.4f \n, ", phiMin,
           phiPhoton, phiMax);

    if (rMin > rMax) {
      Printf("NB!!!!!!!! exit checkUnder(%s) :: rMin %.2f > rMax %.2f ",
             hadronType, rMin, rMax);
    }
  ...
  bool checkOver(const TVector2 &posPhoton, const double &rPhoton,
                 const double &phiPhoton, vecArray3 &vec, const double &etaCkov,
                 const char *hadronType) { // vec : contains phiL, phi, R w
                                           // etaMin/etaMax for hadron species
    ...
    Printf(
        " Enter checkOver(%s) \n initValue = %d \n  vec[initValue-1][1] = %.3f "
        "|| phi2 = vec[initValue][1] =  %.3f |  vec[initValue+1][1] = %.3f;",
        hadronType, initValue, vec[initValue - 1][1], vec[initValue][1],
        vec[initValue + 1][1]);
  ...
  bool checkUnder(const TVector2 &posPhoton, const double &rPhoton,
                  const double &phiPhoton, vecArray3 &vec,
                  const double &etaCkov, const char *hadronType) {
    // vec : contains phiL, phi, R w etaMin/etaMax for hadron species
  ...
  double getR(const double &etaTRS, const double &phiTRS, const double &L) {
    ...
    const TVector2 &rPosLORS =
        tracePhot(etaTRS, phiTRS, L); // pos of estimated rPos at LORS PC
    Printf("\n enter  getR()  rPosLORS {x %.2f y %.2f} - MIP {x %.2f y %.2f}",
           rPosLORS.X(), rPosLORS.Y(), (getPcImp()).X(), (getPcImp()).Y());

    const auto R = (rPosLORS - getPcImp()).Mod();
    Printf("\n exit : getR() : etaTRS %.2f | phiTRS %.2f | L %.2f | R %.2f ",
           etaTRS, phiTRS, L, R);
    return R;
  }

  void splitPhi(double &phiMax, double &phiLmax, double &phiMin,
                double &phiLmin, const double &etaCkov,
                const double &phiPhoton) {
    ...
    Printf("\n enter : splitPhi() \n phiMax %.2f , phiMin  %.2f ", phiMax,
           phiMin);

    Printf("	splitPhi() etaCkov %.2f | phiNew %.2f | phiP %.2f | thetaP "
           "%.2f | phiPhoton %.2f ",
           etaCkov, phiNew, phiP, thetaP, phiPhoton);
    ...
    if ((phiPhoton) < TMath::Pi() + phiP) {
      ...
      if ((phiNew - phiP) > (phiPhoton - phiP)) {
        phiMin = phiNew;
        phiLmin = phiNewL;
        Printf("	splitPhi : if((phiPhoton) < TMath::Pi() + phiP --> "
               "(phiNew-phiP) > (phiPhoton-phiP)");
      } else {
        phiMax = phiNew;
        phiLmax = phiNewL;
        Printf(
            "	splitPhi : if((phiPhoton) < TMath::Pi() + phiP --> else()");
      }
    } else {
      if ((phiNew - phiP) < (phiPhoton - phiP)) {
        Printf("	splitPhi : if((phiPhoton) > TMath::Pi() + phiP --> "
