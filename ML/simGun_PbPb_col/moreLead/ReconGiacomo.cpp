...
protected:
  ...
  // int    *fPhotClusIndex;                     // cluster index of photon
  // candidates
  ...
  o2::hmpid::Param *fParam =
      o2::hmpid::Param::instance(); // Pointer to HMPIDParam
...
public:
  ReconG(double)
    ...
    TVector3 pc(cluX, cluY,
                0.5 * fParam->winThick() + fParam->gapIdx()); // mip at PC
    double cluR =
        TMath::Sqrt((cluX - fPc.X()) * (cluX - fPc.X()) +
                    (cluY - fPc.Y()) *
                        (cluY - fPc.Y())); // ref. distance impact RAD-CLUSTER
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  bool findPhotCkov2(double cluX, double cluY, double &thetaCer,
                     double &phiCer) {
    ...
    TVector3 emissionV(fTrkPos.X(), fTrkPos.Y(),
                       zRad); // impact point at middle of RAD
  ...
  void findRingGeom(double ckovAng, int level) {
    ...
    //            level   - precision in finding area and portion of ring
    //            accepted (multiple of 50)
    ...
    for (Int_t i = 0; i < kN; i++) {
      if (!first) {
        pos1 = tracePhot(
            ckovAng, Double_t(TMath::TwoPi() * (i + 1) /
      ...
      TVector2 pos2 = tracePhot(ckovAng, Double_t(TMath::TwoPi() * (i + 1) /
      ...
      area +=
          TMath::Abs((pos1 - fMipPos).X() * (pos2 - fMipPos).Y() -
                     (pos1 - fMipPos).Y() *
  ...
  const TVector2 intWithEdge(TVector2 p1, TVector2 p2) {
    ...
    if (pint.X() >= 0 && pint.X() <= fParam->sizeAllX() && pint.X() >= xmin &&
        pint.X() <= xmax && pint.Y() >= ymin && pint.Y() <= ymax) {
      return pint;
    }
    // intersection with high X
    pint.Set((double)(p1.X() + (fParam->sizeAllY() - p1.Y()) / m),
             (double)(fParam->sizeAllY()));
    if (pint.X() >= 0 && pint.X() <= fParam->sizeAllX() && pint.X() >= xmin &&
        pint.X() <= xmax && pint.Y() >= ymin && pint.Y() <= ymax) {
      return pint;
    }
    ...
    if (pint.Y() >= 0 && pint.Y() <= fParam->sizeAllY() && pint.Y() >= ymin &&
        pint.Y() <= ymax && pint.X() >= xmin && pint.X() <= xmax) {
      return pint;
    }
    // intersection with righ Y
    pint.Set((double)(fParam->sizeAllX()),
             (double)(p1.Y() + m * (fParam->sizeAllX() -
                                    p1.X()))); // ef: Set->SetCoordinates
    if (pint.Y() >= 0 && pint.Y() <= fParam->sizeAllY() && pint.Y() >= ymin &&
        pint.Y() <= ymax && pint.X() >= xmin && pint.X() <= xmax) {
      return pint;
    }
  ...
  TVector2 getMip() const { return fMipPos; } // mip coordinates
