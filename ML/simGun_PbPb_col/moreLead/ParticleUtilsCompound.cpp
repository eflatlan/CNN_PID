...
public:
  struct Candidate2 {
    ...
    // double R = 0.;
    // double phiL = 0., phi = 0.;
    /*uint32_t */ int candStatus =
        0; //(000, 001, 010, 011, 100, 110, 101, 111);
  };
  ...
  static void
  saveParticleInfoToHDF5(std::vector<ParticleInfo> &particleVector) {
    ...
    for (size_t i = 0; i < particleVector.size(); ++i) {
      ...
      particle.protonCandidatesX.size()); printf("Size of protonCandidatesY:
    ...
    */
      ...
      Attribute attribute = particleGroup.createAttribute(
          "Momentum", PredType::NATIVE_FLOAT, attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.momentum);

      attribute = particleGroup.createAttribute("Mass", PredType::NATIVE_FLOAT,
                                                attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.mass);

      attribute = particleGroup.createAttribute(
          "Energy", PredType::NATIVE_FLOAT, attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.energy);

      attribute = particleGroup.createAttribute(
          "RefractiveIndex", PredType::NATIVE_FLOAT, attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.refractiveIndex);

      attribute = particleGroup.createAttribute("Ckov", PredType::NATIVE_FLOAT,
                                                attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.ckov);

      attribute = particleGroup.createAttribute("xRad", PredType::NATIVE_FLOAT,
                                                attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.xRad);

      attribute = particleGroup.createAttribute("yRad", PredType::NATIVE_FLOAT,
                                                attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.yRad);

      attribute = particleGroup.createAttribute("xPC", PredType::NATIVE_FLOAT,
                                                attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.xPC);

      attribute = particleGroup.createAttribute("yPC", PredType::NATIVE_FLOAT,
                                                attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.yPC);

      attribute = particleGroup.createAttribute(
          "ThetaP", PredType::NATIVE_FLOAT, attr_dataspace);
      attribute.write(PredType::NATIVE_FLOAT, &particle.thetaP);

      attribute = particleGroup.createAttribute("PhiP", PredType::NATIVE_FLOAT,
                                                attr_dataspace);
      ...
      DataSet array_dataset = particleGroup.createDataSet(
          "ArrayInfo", PredType::NATIVE_INT, array_space);
      ...
      // PionCandidates
      DataSet vector_dataset = particleGroup.createDataSet(
          "PionCandidatesX", PredType::NATIVE_DOUBLE, vector_space);
      vector_dataset.write(particle.pionCandidatesX.data(),
                           PredType::NATIVE_DOUBLE);

      vector_dataset = particleGroup.createDataSet(
          "PionCandidatesY", PredType::NATIVE_DOUBLE, vector_space);
      vector_dataset.write(particle.pionCandidatesY.data(),
                           PredType::NATIVE_DOUBLE);
      ...
      vector_dataset = particleGroup.createDataSet(
          "KaonCandidatesX", PredType::NATIVE_DOUBLE, vector_space);
      vector_dataset.write(particle.kaonCandidatesX.data(),
                           PredType::NATIVE_DOUBLE);

      vector_dataset = particleGroup.createDataSet(
          "KaonCandidatesY", PredType::NATIVE_DOUBLE, vector_space);
      vector_dataset.write(particle.kaonCandidatesY.data(),
                           PredType::NATIVE_DOUBLE);
      ...
      vector_dataset = particleGroup.createDataSet(
          "ProtonCandidatesX", PredType::NATIVE_DOUBLE, vector_space);
      vector_dataset.write(particle.protonCandidatesX.data(),
                           PredType::NATIVE_DOUBLE);

      vector_dataset = particleGroup.createDataSet(
          "ProtonCandidatesY", PredType::NATIVE_DOUBLE, vector_space);
      vector_dataset.write(particle.protonCandidatesY.data(),
                           PredType::NATIVE_DOUBLE);

      /* The cands-combined */

      H5::CompType mtype(sizeof(Candidate2));
      mtype.insertMember("x", HOFFSET(Candidate2, x),
                         H5::PredType::NATIVE_DOUBLE);
      mtype.insertMember("y", HOFFSET(Candidate2, y),
                         H5::PredType::NATIVE_DOUBLE);
      mtype.insertMember("candStatus", HOFFSET(Candidate2, candStatus),
      ...
      H5::DataSet dataset =
          particleGroup.createDataSet("candsCombined", mtype, dataspace);
      dataset.write(particle.candsCombined.data(), mtype);
    }
  }

  static std::vector<ParticleInfo>
  loadParticleInfoFromHDF5(const std::string &filename) {
    ...
    for (hsize_t i = 0; i < file.getNumObjs(); ++i) {
      ...
      */
      ...
      for (const auto &candidate : candsCombined) {
        std::cout << "x: " << candidate.x << ", y: " << candidate.y
                  << ", candStatus: " << candidate.candStatus << std::endl;
      }
      ...
      printf("particleInfo.arrayInfo %lu %lu %lu %lu \n",
             particleInfo.arrayInfo[0], particleInfo.arrayInfo[1],
             particleInfo.arrayInfo[2], particleInfo.arrayInfo[3]);
