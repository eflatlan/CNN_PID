...
public:
  ReconE(double _xRad, double _yRad, double _theta, double _phi, double _xPC,
         double _yPC)
  ...
  bool findPhotCkov(double cluX, double cluY, double &thetaCer,
    ...
    double zRad = -0.5 * fParam->radThick() -
                  0.5 * fParam->winThick(); // z position of middle of RAD

    math_utils::Vector3D<double> rad(fTrkPos.X(), fTrkPos.Y(),
                                     zRad); // impact point at middle of RAD
    math_utils::Vector3D<double> pc(
        cluX, cluY, 0.5 * fParam->winThick() + fParam->gapIdx()); // mip at PC

    double cluR = TMath::Sqrt(
        (cluX - fTrkPos.X()) * (cluX - fTrkPos.X()) +
        (cluY - fTrkPos.Y()) *
            (cluY - fTrkPos.Y())); // ref. distance impact RAD-CLUSTER
    ...
    while (1) {
      ...
      o2::math_utils::Vector2D<double> posC =
          traceForward(dirCkov); // trace photon with actual angles
      double dist =
          cluR -
          (posC - fTrkPos)
              .Mag2(); // get distance between trial point and cluster position
  ...
  o2::math_utils::Vector2D<double> traceForward(Polar3DVector dirCkov) const {
    ...
    propagate(dirCkov, posCkov,
              -0.5 * fParam->winThick()); // go to RAD-WIN boundary
    refract(dirCkov, fParam->getRefIdx(),
            fParam->winIdx()); // RAD-WIN refraction
    propagate(dirCkov, posCkov,
              0.5 * fParam->winThick()); // go to WIN-GAP boundary
    refract(dirCkov, fParam->winIdx(), fParam->gapIdx()); // WIN-GAP refraction
    propagate(dirCkov, posCkov,
              0.5 * fParam->winThick() + fParam->gapThick()); // go to PC
  ...
  void lors2Trs(Polar3DVector dirCkov, double &thetaCer, double &phiCer) const {
    ...
    ROOT::Math::Rotation3D mtheta(ROOT::Math::RotationY(
        -fTrkDir
             .Theta())); // TRotation mtheta;  mtheta.RotateY(-fTrkDir.Theta());

    ROOT::Math::Rotation3D mphi(
        ROOT::Math::RotationZ(-fTrkDir.Phi())); // mphi.RotateZ(-fTrkDir.Phi());
  ...
  void trs2Lors(math_utils::Vector3D<double> dirCkov, double &thetaCer,
    ...
    ROOT::Math::Rotation3D mtheta(ROOT::Math::RotationY(
        fTrkDir.Theta())); // TRotation mtheta; mtheta.RotateY(fTrkDir.Theta());
  ...
  //            level   - precision in finding area and portion of ring accepted
  (multiple of 50)
  ...
  for (int i = 0; i < kN; i++) {
      if (!first) {
      pos1 = tracePhot(ckovAng, double(TMath::TwoPi() * (i + 1) / kN)); // find
  a good trace for the first photon if (pos1.X() == -999) { continue; // no
  area: open ring
      }
      if (!fParam->isInside(pos1.X(), pos1.Y(), 0)) {
          pos1 = intWithEdge(fMipPos, pos1); // find the very first
  intersection... } else { if (!Param::isInDead(1.0f, 1.0f)) {// ef: moved
  method from Param.cxx to h nPoints++;                       // photon is
  accepted if not in dead zone
          }
      }
      first = kTRUE;
      continue;
      }
      math_utils::Vector2D<double> pos2 = tracePhot(ckovAng,
  double(TMath::TwoPi() * (i + 1) / kN)); // trace the next photon if (pos2.X()
  == -999) { continue; // no area: open ring
      }
      ...
      area += TMath::Abs((pos1 - fMipPos).X() * (pos2 - fMipPos).Y() - (pos1 -
  fMipPos).Y() * (pos2 - fMipPos).X()); // add area of the triangle... pos1 =
  pos2;
  }
