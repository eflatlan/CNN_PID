...
public:
 ...
 TH2F* hSignalAndNoiseMap =  nullptr;
  ...
  vecArray2 bgCandMapOutRange, ckovCandMapRange, bgCandMapRange,
      ckovCandMapOutRange;
...
delete 			hSignalAndNoiseMap;
  ...
  void setPopulatePtr(Populate2 *_populatePtr) { populatePtr = _populatePtr; }
  ...
  void drawTotalMap(int &plotNumber, int xMip, int yMip,
                    ...
                    TVector2 minPion0, TVector2 minPionPi, TVector2 maxPion0,
                    TVector2 maxPionPi) {
    ...
    std::unique_ptr<TLine> tLineMIP(new TLine(
        xr - len * TMath::Cos(mipPhi), yr - len * TMath::Sin(mipPhi),
        xr + len * TMath::Cos(mipPhi), yr + len * TMath::Sin(mipPhi)));

    std::unique_ptr<TLine> tLineTRK(
        new TLine(xr - len * TMath::Cos(pcPhi), yr - len * TMath::Sin(pcPhi),
                  xr + len * TMath::Cos(pcPhi), yr + len * TMath::Sin(pcPhi)));

    auto distPC2MIP = (mip - pcCon).Mod();
    auto distPC2MIPunconst = (mip - pcUnc).Mod();

    auto st = Form("distPC2MIP %.2f distPC2MIPunconst %.2f", distPC2MIP,
                   distPC2MIPunconst);

    std::unique_ptr<TH2F> hCkovCandMapRange(
        new TH2F(st, st, 1600, 0, 159, 1440, 0, 143));
    std::unique_ptr<TH2F> hBgCandMapRange(new TH2F(
        "bgCandMapRange", "bgCandMapRange", 1600, 0, 159, 1440, 0, 143));
    std::unique_ptr<TH2F> hCkovCandMapOutRange(
        new TH2F("ckovCandMapOutRange", "ckovCandMapOutRange", 1600, 0, 159,
                 1440, 0, 143));
    std::unique_ptr<TH2F> hBgCandMapOutRange(new TH2F(
        "bgCandMapOutRange", "bgCandMapOutRange", 1600, 0, 159, 1440, 0, 143));
    ...
    std::unique_ptr<TH2F> hSignalMIP(new TH2F("hmip ", "hmip ; x [cm]; y [cm]",
                                              1600, 0., 159., 1440, 0, 143));
    std::unique_ptr<TH2F> hSignalMIPpc(new TH2F(
        "hmip pc", "hmip pc; x [cm]; y [cm]", 1600, 0., 159., 1440, 0, 143));
    ...
    std::unique_ptr<TH2F> testPos(
        new TH2F("t", "mt; x [cm]; y [cm]", 1600, 0, 159, 1440, 0, 143));

    Printf("		fillMapFromVec(mArrAndMap->hMaxPion, arrMaxPion);// "
           "map, array");
    ...
    auto trkPCMap =
        std::make_unique<TH2F>("trkPCMap ", "trkPCMap; x [cm]; y [cm]",
                               160 * 10, 0., 159., 144 * 10, 0, 143);
    auto trkRadMap =
        std::make_unique<TH2F>("trkRadMap ", "trkRadMap; x [cm]; y [cm]",
                               160 * 10, 0., 159., 144 * 10, 0, 143);
    auto trkPCMapUnc =
        std::make_unique<TH2F>("trkPCMap Unco", "trkPCMap Unco; x [cm]; y [cm]",
                               160 * 10, 0., 159., 144 * 10, 0, 143);

    auto MIP = std::make_unique<TH2F>("MIP ", "MIP; x [cm]; y [cm]", 160 * 10,
                                      0., 159., 144 * 10, 0, 143);

    Printf("		trkPCMap %.3f %.3f trkPCMapUnc %.3f %.3f", pcCon.X(),
           pcCon.Y(), pcUnc.X(), pcUnc.Y());
    ...
    Printf("		minPion0 %.3f %.3f maxPion0 %.3f %.3f", minPion0.X(),
           minPion0.Y(), maxPion0.X(), maxPion0.Y());
    Printf("		minPionPi %.3f %.3f maxPionPi %.3f %.3f", minPionPi.X(),
           minPionPi.Y(), maxPionPi.X(), maxPionPi.Y());

    // testPos->Fill(minPion0.X(), minPion0.Y()); 	testPos->Fill(minPionPi.X(),
    // minPionPi.Y()); 	testPos->Fill(maxPion0.X(), maxPion0.Y());
    // testPos->Fill(maxPionPi.X(), maxPionPi.Y());
    ...
    auto textNumPion =
        new TLatex(10, 5, ("Pions: " + std::to_string(numPion)).c_str());
    textNumPion->SetTextSize(0.04);
    textNumPion->Draw("same");

    auto textNumKaon =
        new TLatex(35, 5, (" Kaons: " + std::to_string(numKaon)).c_str());
    textNumKaon->SetTextSize(0.04);
    textNumKaon->Draw("same");

    auto textNumProton =
        new TLatex(55, 5, (" Protons: " + std::to_string(numProton)).c_str());
    textNumProton->SetTextSize(0.04);
    textNumProton->Draw("same");

    auto textNumTotal =
        new TLatex(80, 5, ("Total : " + std::to_string(numTotal)).c_str());
...
public:
  ...
  CkovTools(double radParams[7], double refIndexes[3], double MIP[3],
            ...
            int _trackPdg, const o2::dataformats::MatchInfoHMP &track)
    ...
    Printf(" Track PDG %d %s | CkovTools momentum = %.2f, refFreon = %.2f; "
           "ckovHyps : %.2f %.2f %.2f",
           trackPdg, trackPdgString.c_str(), momentum, nF, ckovHyps[0],
    ...
    */
    ...
    Printf("init Ckovtools \n MIP Root : %f %f %f \n MIP local %f %f", op.Px(),
           op.Py(), op.Pz(), xMipLocal, yMipLocal);
  ...
  segment(std::vector<o2::hmpid::ClusterCandidate> &clusterTrack,
    ...
    if (getProtonStatus()) {
      ...
      Printf("calling setArrayMin w getMinCkovProton() = %.2f",
             getMinCkovProton());
      setArrayMin(getMinCkovProton(), arrMinProton, arrMinProtonPos, kN, temp,
                  temp2);

      Printf("calling setArrayMax w getMaxCkovProton() = %.2f",
             getMaxCkovProton());
      setArrayMax(getMaxCkovProton(), arrMaxProton, arrMaxProtonPos, kN, temp,
                  temp2);
    }
    ...
    setArrayMax(getMaxCkovPion(), arrMaxPion, arrMaxPionPos, kN, maxPion0,
                maxPionPi);
    ...
    for (const auto &ip : arrMaxPion) {
      ...
      const auto &r_ = ip[2];
      // Printf("setArrayMax() --> checking arrMaxPionPos | : phiL %.2f, phiR
      // %.2f, r %.2f", phiL_, phiR_, r_);
      if (r_ == 0) {
        throw std::invalid_argument("r====??????;");
      }
    }

    Printf("AFTER : Length of elem vectors : arrMaxPion %zu",
           arrMaxPion.size());

    // Printf(" BF : Length of elem vectors : arrMinPion %zu",
    // arrMinPion.size());
    // Printf("calling setArrayMin w getMinCkovPion() = %.2f",
    // getMinCkovPion());
    ...
    /*
          ...
          inPutVectorAngle.emplace_back(std::array<double, 4>{phiL, phiR, phiPC,
       r}); const auto r = (max - trkPC2).Mod(); // trkPC2 : track impact @ PC
                          // from alinot_paattrec : this is MIP2photon distance?
                          // const auto r = (max - trkPC2).Mod(); // trkPC2 :
       track impact @ PC
          */

    setArrayMin(getMinCkovPion(), arrMinPion, arrMinPionPos, kN, minPion0,
                minPionPi);
    // Printf("AFTER : Length of elem vectors : arrMinPion %zu",
    // arrMinPion.size());
    ...
    if (true) {
      ...
      /*Printf("		fillMapFromVec(mArrAndMap->hMaxPion,
      arrMaxPion);// map, array"); fillMapFromVec(mArrAndMap->hMaxPion,
      arrMaxPionPos);// map, array fillMapFromVec(mArrAndMap->hMaxKaon,
      arrMaxKaonPos);// map, array fillMapFromVec(mArrAndMap->hMaxProton,
      arrMaxProtonPos);// map, array
    ...
    // TODO: ckovHyps : get std-dev for Theta_ckov of pion kaon and proton from
    // the values theta_i
    ...
    // Printf("length ckovPhotons %zu length background %zu length total
    // %zu",cherenkovPhotons.size(), backGroundPhotons.size(),
    // ckovAndBackground.size());
    ...
    for (auto &photons : clusterTrack) {
      ...
      if (photons.mX == mipX &&
        ...
        if (true) {
          ...
          if (photonPDG != mcTrackPdg) {
            ...
            // photons.setIsMip(true, trackIndex); // set the mip to have teh
      ...
      // small radius as we probably will only see secondary charged particles
      // here TODO: ask Giacomo

      /* ef :TODO add this, but what kind of CStatus to define as?
      if(dist < 2) // add small radius where we dont add; this also to not
      include the MIP for hte current track; continue;  // also to not add
      candidates from other MIPs
      */
      ...
      for (const auto &mipCharge : mipCharges) {
        ...
        if (photons.mQ == mipCharge && cStatus == 0) {
          const auto photonPDG = photons.mPDG; // check also other indexes?
          LOGP(info, "CluCharge {} Cluster PDG {} Track {}", photons.mQ,
               photonPDG, mcTrackPdg);

          // ef : this corresponds to the photon being a MIP in the same
          // chamber, for the same event but for a different track
      ...
      Printf(
          "Photon(%.1f %.1f) MIP(%.1f %.1f) RAD (%.1f %.1f) Track (%.1f %.1f)",
          photons.mX, photons.mY, mipX, mipY, xRad, yRad, thetaP, phiP);
      // this means current entry should be a MIP, this will not be a candidate

      // ef : TODO get this value from calibration
      // if(photons.mQ > mipCut) {continue; } // this means current entry should
      // be a MIP, this will not be a candidate
      ...
      /*
Printf("ckovtools cherenkov photons x > xMaxPhiPi && x < xMaxPhi0 && y >
yMaxPhiPi && y < yMaxPhi0"); Printf("ckovtools cherenkov photons x  %f >
xMaxPhiPi %f && x %f < xMaxPhi0 %f && y %f > yMaxPhiPi  %f && y %f < yMaxPhi0
%f",  x, xMaxPhiPi, x , xMaxPhi0 , y , yMaxPhiPi , y , yMaxPhi0);*/

      // Printf("\nckovtools cherenkov photons x  %f > -mL1Max %f && x %f <
      // mL2Max %f && y %f > -mRMax  %f && y %f < mRMax %f\n",  x, -mL1Max, x ,
      // mL2Max , y , -mRMax , y , mRMax);
      ...
      // Printf("\nckovtools cherenkov photons xAbs  %f > mL1Max %f && x %f <
      // mL2Max %f && yAbs %f > mRMax  %f && y %f < mRMax %f\n",  x, mL1Max,
      // xAbs , mL2Max , yAbs , mRMax , y , mRMax);
      ...
      if (rPhoton < rMax && cStatus == 0) {
        ...
        if (getProtonStatus() and getKaonStatus() and getPionStatus()) {
          ...
          isMinProtonOk = populate2Ptr->checkUnder(
              posPhoton, rPhoton, phiPhoton, arrMinProton, getMinCkovProton(),
              "Proton");
          ...
          if (isMinProtonOk) {
            ...
            if (isMaxPionOk) {
              ...
              isMaxProtonOk = populate2Ptr->checkOver(
                  posPhoton, rPhoton, phiPhoton, arrMaxProton,
                  getMaxCkovProton(), "Proton");
              Printf("====================================");
              if (isMaxProtonOk) {
              ...
              if (!isMinKaonOk) {
                ...
                isMinKaonOk = populate2Ptr->checkUnder(
                    posPhoton, rPhoton, phiPhoton, arrMinKaon, getMinCkovKaon(),
                    "Kaon");
              }
              ...
              if (isMinKaonOk) {
                if (!isMaxKaonOk) {
                  isMaxKaonOk = populate2Ptr->checkOver(
                      posPhoton, rPhoton, phiPhoton, arrMaxKaon,
                      getMaxCkovKaon(), "Kaon");
        ...
        // if Proton not is possible because momentum-threshold is not exceeded:
          ...
          Printf("\npopulate2Ptr->checkUnder(posPhoton, rPhoton, phiPhoton, "
                 "arrMinKaon, getMinCkovProton());");
          isMinKaonOk =
              populate2Ptr->checkUnder(posPhoton, rPhoton, phiPhoton,
                                       arrMinKaon, getMinCkovKaon(), "Kaon");
          ...
          if (isMinKaonOk) {
            ...
            if (isMaxPionOk) {
              ...
              isMaxKaonOk =
                  populate2Ptr->checkOver(posPhoton, rPhoton, phiPhoton,
                                          arrMaxKaon, getMaxCkovKaon(), "Kaon");
        ...
        // not exceeded:
          ...
          isMaxPionOk =
              populate2Ptr->checkOver(posPhoton, rPhoton, phiPhoton, arrMaxPion,
                                      getMaxCkovPion(), "Pion");

          if (isMaxPionOk) {

            Printf("populate2Ptr->checkOver(posPhoton, rPhoton, phiPhoton, "
                   "arrMinPion, getMaxCkovPion());",
                   iPhotCount);

            bool isMinPionOk =
                populate2Ptr->checkUnder(posPhoton, rPhoton, phiPhoton,
                                         arrMinPion, getMinCkovPion(), "Pion");
        ...
        Printf("\n============================================================="
               "==");
        Printf("	Photon%d  Candidate: Pion = %d Kaon = %d Proton %d",
               iPhotCount, isPhotonPionCand, isPhotonKaonCand,
               isPhotonProtonCand);
        Printf("==============================================================="
               "\n");
        ...
        if((isPhotonPionCand or isPhotonKaonCand or isPhotonProtonCand) and etaC
        != 0) { numFoundActualCkov ++;
        }

        // correctly identified cherenkov photons
        if(!(isPhotonPionCand or isPhotonKaonCand or isPhotonProtonCand) and
        etaC != 0) { Printf("Ckov %.2f", etaC);
        ...
        if((isPhotonPionCand or isPhotonKaonCand or isPhotonProtonCand) and etaC
        == 0) { numBackgroundLabeledCkov ++;
        }*/

        // struct with x, y, phiLocal, phi, R + bool isCandidate?
        // or just x, y, /* phiLocal,*/ phi, R; where x, y... = 0 if
        // !isCandidate

        // better to not store rPhot and phiPhot? this can easily be obtained in
        // ML photon
        /*pionCands.emplace_back(Candidate{x, y, rPhoton, phiPhoton,
        isPhotonPionCand}); kaonCands.emplace_back(Candidate{x, y, rPhoton,
        phiPhoton, isPhotonKaonCand}); protonCands.emplace_back(Candidate{x, y,
        rPhoton, phiPhoton, isPhotonProtonCand});
        */

        if (cStatus == 0)
          cStatus = 4 * static_cast<int>(isPhotonPionCand) +
        ...
        LOGP(info, "photons.setCandidateStatus(trackIndex {}, cStatus{}); ",
             trackIndex, cStatus);
        /// lagre denne istedet :
        /*
        if( x > 0 && x < 156 && y < 144 && y > 0) {
            candidatesCombined.emplace_back(ParticleUtils::Candidate2{x, y,
        cStatus});
        } */

        // or store as 1 vector, where candidate status 8 => 2^3 (000, 001, 010,
        // 011, 100, 110, 101, 111)
      ...
      if (cStatus == 4 || cStatus == 5 || cStatus == 6 ||
          cStatus == 7) { // 4+1 4+2 4+1+2
...
if(numFoundActualCkov != numActualCkov) {
    ...
    Printf("CkovHyps, possible candidates from Momentum | Pion = %d, Kaon = "
           "%d, Proton = %d",
           getPionStatus(), getKaonStatus(), getProtonStatus());
  ...
  cntTemp++, c.x, c.y, c.candStatus);
    ...
    Printf("number of candidates : proton %zu, kaon %zu, pion %zu | total %zu, "
           "radius over Limit %d ",
           protonCandidates.size(), kaonCandidates.size(),
           pionCandidates.size(), clusterTrack.size(), rOverMax);

    Printf("MIP(%.1f %.1f) RAD (%.1f %.1f) Track (%.1f %.1f)", mipX, mipY, xRad,
           yRad, thetaP, phiP);
    ...
    if (true) {
      ...
      if (false) {
        ...
        mArrAndMap.drawTotalMap(plotNumber, xMip, yMip, pionCandidates,
                                kaonCandidates, protonCandidates, canCombined,
                                minPion0, minPionPi, maxPion0, maxPionPi);

        // mArrAndMap->drawMaxRegions();

        Printf("Event Number%d, momentum %.2f | mass %.2f | thetaP %.2f | phiP "
               "%.2f | L %.2f",
               eventCnt, momentum, mass, thetaP, phiP, L);
        ...
        Printf("phiP %.5f: angle rad --> pc = %.5f | Acos %.5f Asin %.5f", phiP,
               (trkPos - trkPC).Phi(),
               TMath::ACos(d * tanP * cosP / (trkPos - trkPC).Mod()),
               TMath::ASin(d * tanP * sinP / (trkPos - trkPC).Mod()));

        Printf("PHI : l1 %.5f | l2 %.5f ", (l1 - trkPos).Phi(),
               (l2 - trkPos).Phi());
        Printf("RADIUS : l1 %.5f | l2 %.5f ", (l1 - trkPC).Mod(),
               (l2 - trkPC).Mod());
  ...
  void phiRing2Local(double &xL, double &yL, const double &xMipL,
                     const double &yMipL) {
    TRotation mThetaRot;
  ...
  // r0G = rGL + rLP' ; rLP' = Rz(phi)[rLP]
  //  rLP = Rz(-phi) * [r0G-rGL]
  void local2PhiRing(double &xL, double &yL, const double &xMipL,
                     const double &yMipL) {
    TRotation mThetaRot;
    mThetaRot.RotateZ(-phiP);
  ...
  double getR(double etaC, double phiL) {
    ...
    const auto qwDeltaR =
        (qW * nF) / (TMath::Sqrt(nQ * nQ - sinEtaC * sinEtaC * nF * nF));
    const auto num =
        (tGap + tanThetaP * cosPhiL * sinEtaC * (rwlDeltaR + qwDeltaR));

    // ef :error was on this line :
    // 		const auto denum = 1-
    // (tanThetaP*cosPhiL*sinPhiP*nF)/(TMath::Sqrt(nG*nG-sinEtaC*sinEtaC*nF*nF));

    const auto denum =
        1 - (tanThetaP * cosPhiL * sinEtaC * nF) /
                (TMath::Sqrt(nG * nG - sinEtaC * sinEtaC * nF * nF));

    const auto tZ = num / denum;

    const auto Lz = (rW - L) + qW + tZ;

    const auto tGapDeltaR =
        (tZ * nF) / (TMath::Sqrt(nG * nG - sinEtaC * sinEtaC * nF * nF));

    const auto R = sinEtaC * (rwlDeltaR + qwDeltaR + tGapDeltaR) / cosThetaP;
    // Printf("getR_Lmax : R %f |  wlGap %f qwDeltaR %f tGapDeltaR %f", R,
    // rwlDeltaR,qwDeltaR,tGapDeltaR);
    return R;
  }
  ...
  void setArrayMax(double etaTRS, vecArray4 &inPutVectorAngle,
                   vecArray2 &inPutVectorPos, const size_t kN, TVector2 &null,
                   TVector2 &pi) {
    ...
    for (int i = 0; i < kN; i++) {
      ...
      // Printf("setArrayMax() called  populatePtr->traceForward(dirLORS (thetaR
      // %.2f, phiR %.2f) lMin =  %.2f", thetaR, phiR, lMin);
...
std::invalid_argument("r====??????;");}
  ...
  void populateRegions(std::vector<std::pair<double, double>> &vecArr,
    ...
    for (int i = 0; i < vecArr.size(); i++) {
      const auto &value = populatePtr->tracePhot(
          eta, Double_t(TMath::TwoPi() * (i + 1) / kN), l);
