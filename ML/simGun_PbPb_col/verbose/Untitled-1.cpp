...
void Cluster::coG() {
  ...
  for (int iDig = 0; iDig < mDigs->size(); iDig++) { // digits loop
    ...
    mYY +=
        o2::hmpid::Digit::lorsY(padId) * q; // add digit center weighted by QDC
  ...
  mBox = (maxPadX - minPadX + 1) * 100 + maxPadY - minPadY +
         1; // dimension of the box: format Xdim*100+Ydim
  if (mQRaw != 0) {
    mXX /= mQRaw;
    mYY /= mQRaw;
...
void Cluster::corrSin() {
  ...
  int px;
  int py;
  o2::hmpid::Param::lors2Pad(mXX, mYY, pc, px,
                             py);        // tmp digit to get it center
  double x = mXX - param->lorsX(pc, px); // diff between cluster x and center of
                                         // the pad contaning this cluster
  double xpi8on10 = M_PI / 0.8 * x;
  mXX += 3.31267e-2 * sin(2.0 * xpi8on10) - 2.66575e-3 * sin(4.0 * xpi8on10) +
         2.80553e-3 * sin(6.0 * xpi8on10) + 0.0070;
  return;
}
...
*/
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void Cluster::fitFunc(int &iNpars, double *deriv, double &chi2, double *par,
                      int iflag) {
  // Cluster fit function
  // par[0]=x par[1]=y par[2]=q for the first Mathieson shape
  // par[3]=x par[4]=y par[5]=q for the second Mathieson shape and so on up to
  // iNpars/3 Mathieson shapes For each pad of the cluster calculates the
  // difference between actual pad charge and the charge induced to this pad by
  // all Mathieson distributions Then the chi2 is calculated as the sum of this
  // value squared for all pad in the cluster. Arguments: iNpars - number of
  // parameters which is number of local maxima of cluster * 3
  ...
  for (int i = 0; i < nPads; i++) { // loop on all pads of the cluster
    double dQpadMath = 0;
    for (int j = 0; j < iNshape;
         j++) { // Mathiesons loop as all of them may contribute to this pad
      int baseOff = 3 * j;
      int baseOff1 = baseOff + 1;
      int baseOff2 = baseOff + 2;
      double fracMathi = o2::hmpid::Digit::intMathieson(
          par[baseOff], par[baseOff1], pClu->dig(i)->getPadID());
      dQpadMath +=
          par[baseOff2] * fracMathi; // par[3*j+2] is charge par[3*j] is x
                                     // par[3*j+1] is y of current Mathieson
    }
    if (dQpadMath > 0 && pClu->dig(i)->mQ > 0) {
      chi2 += std::pow((pClu->dig(i)->mQ - dQpadMath), 2.0) /
              pClu->dig(i)->mQ; // chi2 function to be minimized
    }
  }
  //---calculate gradients...
  if (iflag == 2) {
    std::vector<std::vector<float>> derivPart(iNpars,
                                              std::vector<float>(nPads, 0.0f));
    ...
    for (int i = 0; i < nPads; i++) { // loop on all pads of the cluster
      ...
      for (int j = 0; j < iNshape;
        ...
        int baseOff2 = baseOff + 2;
        double fracMathi =
            o2::hmpid::Digit::intMathieson(par[baseOff], par[baseOff1], iPadId);
        derivPart[baseOff][i] +=
            par[baseOff2] *
            (o2::hmpid::Digit::mathiesonX(par[baseOff] - lx - dHalfPadX) -
             o2::hmpid::Digit::mathiesonX(par[baseOff] - lx + dHalfPadX)) *
            o2::hmpid::Digit::intPartMathiY(par[baseOff1], iPadId);
        derivPart[baseOff1][i] +=
            par[baseOff2] *
            (o2::hmpid::Digit::mathiesonY(par[baseOff1] - ly - dHalfPadY) -
             o2::hmpid::Digit::mathiesonY(par[baseOff1] - ly + dHalfPadY)) *
...
void Cluster::print(Option_t *opt) const {
  ...
  printf("%sCLU: ch=%i  (X = %7.3f, Y = %7.3f) Q=%8.3f Qraw=%8.3f(%3.0f%%) "
         "Size=%2i DimBox=%i LocMax=%i Chi2=%7.3f   %s\n",
         opt, mCh, mXX, mYY, mQ, mQRaw, ratio, mSi, mBox, mNlocMax, mChi2,
         status);
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
int Cluster::solve(std::vector<o2::hmpid::Cluster> *pCluLst, float *pSigmaCut,
                   bool isTryUnfold) {
  // This methode is invoked when the cluster is formed to solve it. Solve the
  // cluster means to try to unfold the cluster into the local maxima number of
  // clusters. This methode is invoked by AliHMPIDRconstructor::Dig2Clu() on
  // cluster by cluster basis. At this point, cluster contains a list of digits,
  // cluster charge and size is precalculated in AddDigit(), position is preset
  // to (-1,-1) in ctor, status is preset to kFormed in AddDigit(),
  // chamber-sector info is preseted to actual values in AddDigit() Method first
  // finds number of local maxima and if it's more then one tries to unfold this
  // cluster into local maxima number of clusters Arguments: pCluLst     -
  // cluster list pointer where to add new cluster(s)
  ...
  int iCluCnt = pCluLst->size(); // get current number of clusters already
                                 // stored in the list by previous operations
  ...
  if (rawSize > 100 || isTryUnfold == false ||
      rawSize == 1) { // No deconv if: 1 - big cluster (also avoid no zero
                      // suppression!)
    // setClusterParams(mXX, mYY, mCh); //                               2 -
    // flag is set to FALSE new ((*pCluLst)[iCluCnt++]) Cluster(*this); // 3 -
    // size = 1
  ...
  TVirtualFitter *fitter =
      TVirtualFitter::Fitter((TObject *)this, 3 * 6); // initialize Fitter
  if (fitter == nullptr) {
  ...
  arglist[0] = 1;
  ierflg = fitter->ExecuteCommand("SET GRA", arglist,
                                  1); // force Fitter to use my gradient
  fitter->SetFCN(Cluster::fitFunc);
  // Phase 1. Find number of local maxima. Strategy is to check if the current
  // pad has QDC more then all neigbours. Also find the box contaning the
  // cluster
  mNlocMax = 0;
  for (int iDig1 = 0; iDig1 < rawSize; iDig1++) { // first digits loop
    auto pDig1 = (*mDigs)[iDig1];                 // take next digit
    int iCnt =
        0; // counts how many neighbouring pads has QDC more then current one
    for (int iDig2 = 0; iDig2 < rawSize; iDig2++) { // loop on all digits again
      ...
      int dist =
          TMath::Sign(int(pDig1->mX - pDig2->mX), 1) +
          TMath::Sign(int(pDig1->mY - pDig2->mY), 1); // distance between pads
    ...
    if (iCnt == 0 && mNlocMax < kMaxLocMax) { // this pad has Q more then any
                                              // neighbour so it's local maximum
      ...
      ierflg = fitter->SetParameter(
          3 * mNlocMax, Form("x%i", mNlocMax), xStart, 0.1, xMin,
          xMax); // X,Y,Q initial values of the loc max pad
      ierflg = fitter->SetParameter(
          3 * mNlocMax + 1, Form("y%i", mNlocMax), yStart, 0.1, yMin,
          yMax); // X, Y constrained to be near the loc max
  ...
  // Phase 2. Fit loc max number of Mathiesons or add this current cluster to
  // the list case 1 -> no loc max found
  if (mNlocMax ==
      0) { // case of no local maxima found: pads with same charge...
    mNlocMax = 1;
    mSt = kNoLoc;
    // setClusterParams(mXX, mYY, mCh); //need to fill the AliCluster3D part
    pCluLst->push_back(o2::hmpid::Cluster(
        *this)); // add new unfolded cluster
                 // pCluLst->push_back(o2::hmpid::Cluster(*this));
    pCluLst->back().cleanPointers();
    return mNlocMax;
  }
  ...
  if (mNlocMax >= kMaxLocMax) {
    // setClusterParams(mXX, mYY, mCh); // if # of local maxima exceeds
    // kMaxLocMax...
    mSt = kMax;
    ...
    arglist[1] = 1.;  //
    ierflg = fitter->ExecuteCommand("SIMPLEX", arglist,
                                    2); // start fitting with Simplex
    if (!ierflg) {
      fitter->ExecuteCommand("MIGRAD", arglist,
                             2); // fitting improved by Migrad
    }
    if (ierflg) {
      double strategy = 2.;
      ierflg = fitter->ExecuteCommand("SET STR", &strategy,
                                      1); // change level of strategy
      if (!ierflg) {
        ierflg = fitter->ExecuteCommand("SIMPLEX", arglist,
                                        2); // start fitting with Simplex
        if (!ierflg) {
    ...
    for (int i = 0; i < mNlocMax; i++) { // store the local maxima parameters
      ...
      if (mNlocMax > 1) {
        findClusterSize(
            i, pSigmaCut); // find clustersize for deconvoluted clusters
                           // after this call, fSi temporarly is the calculated
                           // size. Later is set again to its original value
      }
      if (mSt != kAbn) {
        if (mNlocMax != 1) {
...
void Cluster::findClusterSize(int i, float *pSigmaCut) {
  ...
  for (int iDig = 0; iDig < mSi; iDig++) { // digits loop
    ...
    int iCh = pDig->mCh;
    double qPad = mQ * o2::hmpid::Digit::intMathieson(
                           x(), y(), pDig->getPadID()); // pad charge  pDig->
    //  AliDebug(1,Form("Chamber %i X %i Y %i SigmaCut %i pad %i qpadMath %8.2f
    //  qPadRaw %8.2f Qtotal %8.2f cluster n.%i",
    //                 iCh, o2::hmpid::Digit::a2X(pDig->getPadID()),
    //                 o2::hmpid::Digit::a2Y(pDig->getPadID()),
...
void Cluster::digAdd(const Digit *pDig) {
  // Adds a given digit to the list of digits belonging to this cluster, cluster
  // is not owner of digits Arguments: pDig - pointer to digit to be added
  // Returns: none
  if (!mDigs) {
