  ...
  segment(std::vector<o2::hmpid::ClusterCandidate> &clusterTrack,
    ...
    for (auto &photons : clusterTrack) {
      ...
      if ((rPhoton < rMax && rPhoton > rMin) && cStatusCkov == 0) {
        ...
        if (findPhotCkov(
          ...
          //
            ...
            if (TMath::Abs(thetaCer - getCkovPion()) < mSigmaSep * sigmaRing) {
              isPhotonPionCandCkov = true; // Printf("ckovPionOk");
              pionCandidates.push_back(std::array<double, 2>{x, y});
            }
            if (TMath::Abs(thetaCer - getCkovKaon()) < mSigmaSep * sigmaRing) {
      ...
      if (getProtonStatus() and getKaonStatus() and getPionStatus()) {
         ...
         * double& phiPhoton, vecArray4& vec, const double& etaCkov, const char*
         * hadronType)
         */
        ...
        if (isMinProtonOk) {
          ...
          // printf("\n\npopulate2Ptr->checkOver(posPhoton, rPhoton, phiPhoton,
          ...
          if (isMaxPionOk) {
            ...
            // this means rProtonMax > rPhoton; then also the rPh < rPionMax and
            // rPh < rKaonMax
            ...
            // printf("		isMaxProtonOk = !evaluatePointContour(posPhoton,
            // arrMaxProtonPos, true)");
      ...
      // if Proton not is possible because momentum-threshold is not exceeded:
        ...
        bool shutDownOnOpen = false;
        // printf("isMinKaonOk = evaluatePointContour(posPhoton,  arrMinKaonPos,
        // false)"); isMinKaonOk = populate2Ptr->checkUnder(posPhoton, rPhoton,
        // phiPhoton, arrMinKaon, getMinCkovKaon(), "Kaon", shutDownOnOpen);
      ...
      // if neither Proton or Kaon is possible because momentum-threshold is not
      // exceeded:
      else if (!getProtonStatus() and !getKaonStatus() and getPionStatus()) {
        ...
        // isMaxPionOk = populate2Ptr->checkOver(posPhoton, rPhoton, phiPhoton,
        // arrMaxPion, getMaxCkovPion(), "Pion");
        isMaxPionOk = !evaluatePointContour(posPhoton, arrMaxPionPos,
                                            true); // inside =  no intersection!
...
if(numFoundActualCkov != numActualCkov) {
  ...
  // printf("=========================================================================");
  // printf("CkovHyps, possible candidates from Momentum | Pion = %d, Kaon = %d,
  // Proton = %d", getPionStatus(), getKaonStatus(), getProtonStatus());
...
cntTemp++, c.x, c.y, c.candStatus);
  ...
  // printf("number of candidates : proton %zu, kaon %zu, pion %zu | total %zu,
  // radius over Limit %d ", protonCandidates.size(), kaonCandidates.size(),
  // pionCandidates.size(), clusterTrack.size(), rOverMax);
  ...
  if (/*drawMap*/ false) {
    ...
    if (false) { // printf("populate2Ptr was nullptr!");
      ...
      Printf("SigmaSep %.2f  | nPi %d nK %d nPr %d == nT %d | PDG %d | thetaP "
             "%.2f phiP %.2f",
             mSigmaSep, numPion, numKaon, numProton, numTotal, trackPdg, thetaP,
             phiP);
      ...
      mArrAndMap.drawTotalMap(clusterTrack, plotNumber, xMip, yMip,
                              pionCandidates, kaonCandidates, protonCandidates,
                              canCombined, trackPdg, allCand, mSigmaSep);
      // to drqw the maps ::
...
void setArrayMax(double etaTRS, vecArray4 &inPutVectorAngle,
  ...
  for (int i = 0; i < kN; i++) {
    ...
    // Printf("setArrayMax() called trs2Lors on dirTRS : return { thetaR = %.2f,
    // phiR = %.2f}", thetaR, phiR);
    ...
    const auto r =
        (max - trkPC2).Mod(); // trkPC2 : track impact @ PC
                              ...
                              // distance? const auto r = (max - trkPC2).Mod();
                              // // trkPC2 : track impact @ PC
    ...
    // Printf("setArrayMax2() --> i %d, {x %.2f y %.2f} - MIP {x %.2f y %.2f}",
    // i, max.X(), max.Y(), trkPC2.X(), trkPC2.Y());
   ...
   for(const auto& ip : inPutVectorAngle) {
           ...
           const auto& r_ = ip[2];
           //Printf("setArrayMax() --> checking inputVector | : phiL %.2f, phiR
%.2f, r %.2f", phiL_, phiR_, r_); if(r_ == 0 ) {throw
...
void calculateDifference(vecArray2 &maxVec, vecArray2 &minVec,
  ...
  for (size_t i = 0; i < kN; ++i) {
    ...
    // Printf(" phiP TRS_deg %.2f | thetaP_deg %.2f ckovHypVal %.2f | sigma %.5f
    // sigma2Ali %.5f ", phiL*180/3.14, thetaP*180/3.14,  ckovHypVal, sigma2,
    // sigma2Ali);
...
double getTotalSigmaPos(double sigmaPositions) {
  ...
  Printf(
      " getTotalSigmaPos : sigmaPositionssq %.3f  sigmaLsq %.23f sigmaRsq %.3f",
...
Double_t aliSigma2(Double_t trkTheta, Double_t trkPhi, Double_t ckovTh,
  ...
  if (sigRMS < 0.00000002)
    Printf("SigLoc %.5f, SigGeom %.5f, z (SigCrom): %.5f  == %.5f (sigRMS): | "
           "Inputs: trkTheta: %.5f, trkPhi: %.5f, ckovTh: %.5f, ckovPh: %.5f, "
           "trkBeta: %.5f",
...
Double_t SigLoc(Double_t trkTheta, Double_t trkPhi, Double_t thetaC,
  ...
  // Analitical calculation of localization error (due to finite segmentation of
  // PC) on Cerenkov angle for a given Cerenkov photon created by a given MIP.
  // Fromulae according to CERN-EP-2000-058 Arguments: Cerenkov and azimuthal
  // angles for Cerenkov photon, [radians]
  //            dip and azimuthal angles for MIP taken at the entrance to
  //            radiator, [radians] MIP beta
  //   Returns: absolute error on Cerenkov angle, [radians]
  ...
  Double_t k = 1. - GetRefIdx() * GetRefIdx() +
               alpha * alpha / (betaM * betaM); // formula (after 8 in the text)
  if (k < 0)
    return 1e10;
  Double_t mu = sint * sinf +
  ...
  Double_t dtdyc =
      kk * (k * (cosfd * sinf + cost * sinfd * cosf) +
            (alpha * e / (betaM * betaM)) * sint * sinfd); // formula (7) pag.4
...
Double_t SigGeom(Double_t trkTheta, Double_t trkPhi, Double_t thetaC,
  ...
  Double_t c1 =
      1. /
      (1. + eTr * k / (alpha * alpha * costheta * costheta)); // formula (13.a)
...
void populateRegions(std::vector<std::pair<double, double>> &vecArr, TH2F *map,
                     const double &eta, const double &l) {
...
bool findPhotCkov(double cluX, double cluY, double &thetaCer, double &phiCer) {
  ...
  */
  ...
  double cluR =
      TMath::Sqrt((cluX - mipPos.X()) * (cluX - mipPos.X()) +
                  (cluY - mipPos.Y()) *
