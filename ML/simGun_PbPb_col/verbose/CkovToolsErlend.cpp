...
delete 			hSignalAndNoiseMap;
  ...
  void drawTotalMap(std::vector<o2::hmpid::ClusterCandidate> &clusterTrack,
                    ...
                    int trackPdg, vecArray2 allCand) {
    ...
    Printf(" (mip-trkRad) %.2f(trkPC-trkRad)  %.2f", (mip - trkRad).Phi(),
           (trkPC - trkRad).Phi());
    // Printf(" (mip-trkRad) { %.2f,  %.2f}  (trkPC-trkRad) { %.2f,  %.2f}",
    // (mip-trkRad).Phi(), (mip-trkRad).Theta(), (trkPC-trkRad).Phi(),
    // (trkPC-trkRad).Theta());
    auto len1 = 15.;
    auto len2 = 10.;
    ...
    // pcPhi %.2f", trackPdg, numPion, numKaon, numProton, numTotal, distPC2MIP,
    // mipPhi, pcPhi);
    auto st = Form(" pdg %d | \#Pi %d K %d P %d T %d | \n | pcPhi %.2f",
                   trackPdg, numPion, numKaon, numProton, numTotal, pcPhi);
    ...
    std::unique_ptr<TH2F> totalCluMap(new TH2F("All clusters",
                                               "All clusters ; x [cm]; y [cm]",
                                               320, 0, 159, 288, 0, 143));
...
public:
  ...
  CkovTools(double radParams[7], double refIndexes[3], double MIP[3],
            ...
            float trackCkov, int eventCnt, int _trackPdg)
  ...
  segment(std::vector<o2::hmpid::ClusterCandidate> &clusterTrack,
    ...
    if (getProtonStatus()) {
      ...
      calculateDifference(arrMaxProtonPos, arrMinProtonPos, getMaxCkovProton(),
                          getMinCkovProton());
    }
    ...
    if (getKaonStatus()) {
      ...
      // printf("calling setArrayMin w getMinCkovKaon() = %.2f",
      // getMinCkovKaon());
      calculateDifference(arrMaxKaonPos, arrMinKaonPos, getMaxCkovKaon(),
                          getMinCkovKaon());
    }
    ...
    calculateDifference(arrMaxPionPos, arrMinPionPos, getMaxCkovPion(),
                        getMinCkovPion());
    ...
    for (auto &photons : clusterTrack) {
      ...
      if ((rPhoton < rMax && rPhoton > rMin) && cStatus == 0) {
        ...
        // iPhotCount, isPhotonPionCand, isPhotonKaonCand, isPhotonProtonCand);
        // printf("===============================================================\n");
        ...
        LOGP(debug, "photons.setCandidateStatus(trackIndex {}, cStatus{}); ",
             trackIndex, cStatus);
    ...
    if (/*drawMap*/ true) {
      ...
      if (false) { // printf("populate2Ptr was nullptr!");
        ...
        mArrAndMap.drawTotalMap(
            clusterTrack, plotNumber, xMip, yMip, pionCandidates,
            kaonCandidates, protonCandidates, canCombined, trackPdg, allCand);
        // to drqw the maps ::
...
std::invalid_argument("r====??????;");}
  ...
  void calculateDifference(vecArray2 &maxVec, vecArray2 &minVec,
                           double etaTrsMax, double etaTrsMin) {
    ...
    for (size_t i = 0; i < kN; ++i) {
      ...
      if (maxPos.X() < -800 || maxPos.Y() < -800) {
        if (minPos.X() < -800 || minPos.Y() < -800) {
          ...
          setMinSingle(meanPos, etaTrsMean,
                       phiL); // setMaxSingle(TVector2& vectorOut, double
                              // etaTRS, double phiL);
          if (meanPos.X() < -800 || meanPos.Y() < -800) {
            ...
            TVector2 temp;
            auto etaDiff = 0.001;
            setMeanSingle(temp, etaTrsMin - etaDiff,
                          phiL); // setMaxSingle(TVector2& vectorOut, double
                                 // etaTRS, double phiL);
            ...
            auto rDelta =
                (etaTrsMean - etaTrsMin) * (drDetaMin - drDetaTemp) / etaDiff;
            ...
            Printf("	Outer open Mean open :  interpolated  drDetaMin %.2f "
                   "drDetaTemp %.2f rDelta %.2f",
                   drDetaMin, drDetaTemp, rDelta);
            ...
            Printf("	Outer open Mean closed :  phiMin %.4f  phiMean %.4f",
                   phiMin, phiMean);
            ...
            Printf("	rMin %.2f  rMean %.2f | rDeltaMin %.2f | sigmaPos %.2f",
                   rMin, rMean, rDeltaMin, sigmaPos);
            Printf("	minPos.X() %.2f  dx %.2f ", minPos.X(), dx);
            Printf("	minPos.y() %.2f  dy %.2f ", minPos.Y(), dy);
        ...
        double etaTrsMean = (etaTrsMax + etaTrsMin) / 2.;
        {
          ...
          Printf("	rMin %.2f  rMean %.2f | rDeltaMin %.2f | sigmaPos %.2f "
                 "newR %.2f",
                 rMin, rMean, rDeltaMin, sigmaPos, newR);
...

        {
          ...
          Printf("	rMax %.2f  rMean %.2f | rDeltaMax %.2f | sigmaPos %.2f",
                 rMax, rMean, rDeltaMax, sigmaPos);
  ...
  double getTotalSigmaPos(double sigmaPositions) {
    Printf(" getTotalSigmaPos : sigmaPositions %.2f  sigmaL %.2f sigmaR %.2f",
           sigmaPositions, TMath::Sqrt(sigmaLsq), TMath::Sqrt(sigmaRsq));
    // sigmaAngles sigmaE sigmaThetaP --- sigmaPositions
    // Printf(" getTotalSigmaPos : sigmaPositionssq %.2f  sigmaLsq %.2f sigmaRsq
    // %.2f", sigmaPositions * sigmaPositions, sigmaLsq, sigmaRsq);
