...
protected:
  ...
  math_utils::Vector2D<double> fTrkPos; // track positon in LORS at RAD   // XY
  mag math_utils::Vector2D<double> fMipPos; // mip positon for a given trackf //
  XY math_utils::Vector2D<double> fPc;     // track position at PC           //
  XY
  */
...
public: //   ReconG reconG(thetaP, phiP, xP, yP, xPC, yPC, nF);
  ReconG(double _theta, double _phi, double _xRad, double _yRad, double _xPC,
         double _yPC, double n, double _etaC)
      : refIdx(n), etaC(_etaC) {
  ...
  /*    ReconG(double _xRad, double _yRad, double _theta, double _phi, double
     _xPC, double _yPC) : / *fParam(o2::hmpid::Param::instance()) * /
      {
        setTrack(_xRad, _yRad, _theta, _phi);
        setImpPC(_xPC, _yPC);
      }*/
  ...
  bool findPhotCkov(double cluX, double cluY, double &thetaCer, double &phiCer,
    ...
    // double zRad = -0.5 * radThick - 0.5 * winThick;   // z position of middle
    // of RAD
    ...
    while (1) {
      ...
      // TODO: nb!! cant really see where cosEtaC = sinThetaP *cos(phi-phiP)..
      // is done??
      if (posC.X() == -999) {
        dist = -999;
      ...
      //	Printf("ReconG findCkov: cnt %d| ckov %.4f : ckov1 %.4f : ckov2
      //%.4f | dist %.3f" , iIterCnt, ckov, ckov1, ckov2, dist);
    ...
    const auto infString4 =
        Form("ReconG meth: | Actual Ckov : %.3f | Reconstructed Ckov1 = %.3f",
             etaC, thetaCer);
    TCanvas *tCkovGraphG =
        new TCanvas("T ReconG", "etaC Graph ReconG", 1600, 1600);
    tCkovGraphG->cd();
