...
public:
  TH2F *getLimMin() { return limMin; }

  TH2F *getLimMax() { return limMax; }
...
limMin = new TH2F("minPos","minPos", 1600, 0, 159, 1440, 0, 143);
  ...
  TVector3 getTrkDir() const { return fTrkDir; }
  ...
  bool checkOver(const TVector2 &posPhoton, const double &rPhoton,
    ...
    while (/*phi2 < phiPhoton*/ true) {
      ...
      if (initValue + iCnt + 1 >= kN) { // do vec size instead?
        ...
        return true; // false
        // We have open contour, or contour goes out of map --> The particle
        // should be l labelled as being a candidate
...

    {
      ...
      Printf("	newPos1 = %.2f,  %.2f ||newPos2 = %.2f, %.2f", newPos1.X(),
             newPos1.Y(), newPos2.X(), newPos2.Y());
      limMax->Fill(newPos1.X(), newPos1.Y());
      limMin->Fill(newPos2.X(), newPos2.Y());
      Printf("	rPhoton = %.2f, rMax = %.2f, rMin = %.2f || from getR :  rMax "
             "= %.2f, rMin = %.2f",
             rPhoton, rMax, rMin, rMaxCp, rMinCp);
    }
    ...
    while (!decisionTaken) {
      ...
      if ((rPhoton < rMin + uncSquared)) {
        ...
        Printf("rPhoton %.2f < rMin + uncSquared %.2f", rPhoton,
               rMin + uncSquared);
        // stop iterating, condition is ok!
        decisionTaken = true;
        condition = true;
        ...
        Printf("rPhoton %.2f > rMax + uncSquared %.2f", rPhoton,
               rMax + uncSquared);
        // stop iterating, condition is false
        decisionTaken = true;
        condition = false;
        ...
        rMin = getR(newPos2, etaCkov, phiLmin,
                    lMin); // TODO: should it be lMin here?
                           // limMax->Fill(newPos1.X(), newPos1.Y());
        // limMin->Fill(newPos2.X(), newPos2.Y());
  ...
  double getR(TVector2 &rPosLORS, const double &etaTRS, const double &phiTRS,
              const double &L) {
