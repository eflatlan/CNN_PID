...
std::vector<ParticleInfo2> readH5File(const std::string& filename) {
    std::vector<ParticleInfo2> particleInfos;
    H5::H5File file(filename, H5F_ACC_RDONLY);

    // Read number of particles/groups
    // Assuming each group in the file is a particle
    // This part may need to be adjusted based on actual file structure
    H5::Group root_group = file.openGroup("/");
    int num_particles = root_group.getNumObjs(); 

    for (int i = 0; i < num_particles; ++i) {
        std::string groupName = "Particle" + std::to_string(i);
        H5::Group particleGroup = file.openGroup(groupName);

        // Read attributes and data for each particle
        // This needs to be aligned with the structure of your H5 file

        // Example for reading an attribute
        double xRad, yRad, xMip, yMip, thetaP, phiP;
        H5::Attribute attribute = particleGroup.openAttribute("xRad");
        attribute.read(H5::PredType::NATIVE_DOUBLE, &xRad);

        // Repeat for other attributes...

        // Read cluster data (adjust according to actual dataset names and structures)
        // ...

        // Create ParticleInfo2 objects and fill with read data
        ParticleInfo2 pInfo; // Assuming ParticleInfo2 can be initialized with these values
        // Fill pInfo with read data
        particleInfos.push_back(pInfo);
    }
